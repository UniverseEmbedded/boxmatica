...
import pama1234.boxmatica.gui.GuiMainMenu;
import pama1234.boxmatica.gui.GuiMaterialList;
import pama1234.boxmatica.gui.GuiPlacementConfiguration;
import pama1234.boxmatica.gui.GuiRenderLayer;
import pama1234.boxmatica.gui.GuiSchematicLoadedList;
import pama1234.boxmatica.gui.GuiSchematicPlacementsList;
import pama1234.boxmatica.gui.GuiSchematicVerifier;
import pama1234.boxmatica.gui.GuiSubRegionConfiguration;
...
import pama1234.boxmatica.selection.CornerSelectionMode;
    ...
    public boolean onKeyAction(KeyAction action,IKeybind key) {
      ...
      if(key==Hotkeys.EASY_PLACE_ACTIVATION.getKeybind()) {
        ...
        if(DataManager.getSchematicProjectsManager().hasProjectOpen()==false) {
          ...
          InfoUtils.showGuiOrInGameMessage(MessageType.WARNING,"boxmatica.gui.button.hover.schematic_projects.area_browser_disabled_currently_in_projects_mode");
        }
        ...
        SchematicWorldRefresher.INSTANCE.updateAll();
        InfoUtils.printActionbarMessage("boxmatica.message.schematic_rendering_refreshed");
        return true;
      ...
      if(key==Hotkeys.ADD_SELECTION_BOX.getKeybind()) {
        ...
        if(mode.getUsesAreaSelection()) {
          ...
          if(selection!=null) {
            if(selection.isOriginSelected()) {
              ...
              InfoUtils.printActionbarMessage("boxmatica.message.removed_area_origin");
            }else {
              String name=selection.getCurrentSubRegionBoxName();

              if(name!=null&&selection.removeSelectedSubRegionBox()) {
                InfoUtils.printActionbarMessage("boxmatica.message.removed_selection_box",name);
                return true;
              }
            }
          }
        ...
        if(placement!=null) {
          ...
          if(placement.isLocked()) {
            ...
            InfoUtils.printActionbarMessage("boxmatica.message.placement.rotation_set_to",PositionUtils.getRotationNameShort(rotation));
          }
          return true;
        }
        ...
        if(placement!=null) {
          ...
          if(placement.isLocked()) {
            ...
            InfoUtils.printActionbarMessage("boxmatica.message.placement.mirror_set_to",PositionUtils.getMirrorName(mirror));
          }
          return true;
        }
        ...
        if(mode.getUsesAreaSelection()) {
          ...
          if(area!=null) {
            ...
            InfoUtils.printActionbarMessage("boxmatica.message.set_area_origin",posStr);
            return true;
          }
        }
        ...
        key==Hotkeys.SET_SELECTION_BOX_POSITION_2.getKeybind()) {
          if(mode.getUsesAreaSelection()) {
            ...
            if(area!=null&&area.getSelectedSubRegionBox()!=null) {
              ...
              String posStr=String.format("x: %d, y: %d, z: %d",pos.getX(),pos.getY(),pos.getZ());
              InfoUtils.printActionbarMessage("boxmatica.message.set_selection_box_point",corner.ordinal(),posStr);
              return true;
            }
          }
        }
