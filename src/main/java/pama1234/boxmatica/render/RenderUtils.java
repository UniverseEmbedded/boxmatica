package pama1234.boxmatica.render;
...
import pama1234.boxmatica.util.BlockInfoAlignment;
import pama1234.boxmatica.util.InventoryUtils;
...
public class RenderUtils{
  ...
  //        catch (Exception e)
  //        {
  //            Boxmatica.LOGGER.error("renderBlockOutline: Failed to draw Area Selection box (Error: {})", e.getLocalizedMessage());
  //        }
  //    }
  ...
  //        catch (Exception e)
  //        {
  //            Boxmatica.LOGGER.error("renderBlockOutlineOverlapping: Failed to draw Area Selection box (Error: {})", e.getLocalizedMessage());
  //        }
  //    }
  ...
  public static int renderInventoryOverlay(BlockInfoAlignment align,LeftRight side,int offY,
    ...
    if(ctx!=null&&ctx.inv()!=null) {
      ...
      //            Boxmatica.LOGGER.error("render(): type [{}], inv [{}], be [{}], nbt [{}]", ctx.type().name(), ctx.inv().size(), ctx.be() != null, ctx.nbt() != null ? ctx.nbt().getString("id") : new NbtCompound());
  ...
  /*
   * private static void renderModelBrightnessColor(IBlockState state, IBakedModel model, float
   ...
   * }
   * 
   * private static void renderModelBrightnessColorQuads(float brightness, float red, float green,
   * float blue, List<BakedQuad> listQuads)
   * {
   * Tessellator tessellator = Tessellator.getInstance();
  ...
  /*
   * private static void renderModel(final IBlockState state, final IBakedModel model, final
   * BlockPos pos, final int alpha)
   * {
   ...
   * }
   * 
   * private static void renderQuads(final IBlockState state, final BlockPos pos, final
   * BufferBuilder buffer, final List<BakedQuad> quads, final int alpha)
   * {
   * final int size = quads.size();
   * 
   * for (int i = 0; i < size; i++)
   ...
   * final int color = quad.getTintIndex() == -1 ? alpha | 0xffffff : getTint(state, pos, alpha,
   * quad.getTintIndex());
   ...
   * }
   * 
   * private static int getTint(final IBlockState state, final BlockPos pos, final int alpha,
   * final int tintIndex)
   * {
