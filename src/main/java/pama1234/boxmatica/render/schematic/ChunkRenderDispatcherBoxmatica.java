...
public class ChunkRenderDispatcherBoxmatica{
  private static final Logger LOGGER=Boxmatica.LOGGER;
  // Threaded Code
  //private static final ThreadFactory THREAD_FACTORY = (new ThreadFactoryBuilder()).setNameFormat("Boxmatica Chunk Batcher %d").setDaemon(true).build();

  private final List<Thread> listWorkerThreads=new ArrayList<>();
  private final List<ChunkRenderWorkerBoxmatica> listThreadedWorkers=new ArrayList<>();
  ...
  private final Queue<ChunkRenderDispatcherBoxmatica.PendingUpload> queueChunkUploads=Queues.newPriorityQueue();
  //    final Queue<Runnable> queueChunkUploads = Queues.newConcurrentLinkedQueue();
  private final ChunkRenderWorkerBoxmatica renderWorker;
  private final int countRenderAllocators;
  // Threaded Code
  //private final int countRenderThreads;
  private Vec3d cameraPos;

  public ChunkRenderDispatcherBoxmatica(Profiler profiler) {
    /*
     * Threaded Code
     * 
     * int threadLimitMemory = Math.max(1, (int) ((double) Runtime.getRuntime().maxMemory() * 0.3D)
     * / BufferAllocatorCache.EXPECTED_TOTAL_SIZE);
     * int threadLimitCPU = Math.max(1, MathHelper.clamp(Runtime.getRuntime().availableProcessors(),
     * 1, threadLimitMemory / 5));
     * int maxThreads = Math.max(1,
     * Math.min((Configs.Visuals.RENDER_SCHEMATIC_MAX_THREADS.getIntegerValue()), threadLimitCPU));
     * int maxCache = Math.max(1,
     * Math.min((Configs.Visuals.RENDER_SCHEMATIC_MAX_THREADS.getIntegerValue() - 1),
     * threadLimitMemory));
     ...
     * {
     * ChunkRenderWorkerBoxmatica worker = new ChunkRenderWorkerBoxmatica(this);
     ...
     * {
     * LOGGER.warn("Only able to allocate {}/{} BufferAllocator caches",
     * this.queueFreeRenderAllocators.size(), i);
     * int adjusted = Math.min(this.queueFreeRenderAllocators.size() * 2 / 3,
     * this.queueFreeRenderAllocators.size() - 1);
     * 
     ...
     * 
     * this.renderWorker = new ChunkRenderWorkerBoxmatica(this, new BufferAllocatorCache());
     */
    ...
    this.renderWorker=new ChunkRenderWorkerBoxmatica(this,new BufferAllocatorCache(),profiler);
  }
  ...
  protected boolean updateChunkLater(ChunkRendererSchematicVbo renderChunk,Profiler profiler) {
     ...
     * renderChunk.makeCompileTaskChunkSchematic(this::getCameraPos);
    ...
    try {
      ...
      generator.addFinishRunnable(new Runnable() {
        public void run() {
          ChunkRenderDispatcherBoxmatica.this.queueChunkUpdates.remove(generator);
        }
      });
  ...
  protected boolean updateChunkNow(ChunkRendererSchematicVbo chunkRenderer,Profiler profiler) {
     ...
     * catch (InterruptedException e)
     * {
     * LOGGER.warn("updateChunkNow(): Process Interrupted; error message: [{}]",
  ...
  protected ListenableFuture<Object> uploadChunkBlocks(final RenderLayer layer,final BufferAllocatorCache allocators,final ChunkRendererSchematicVbo renderChunk,final ChunkRenderDataSchematic chunkRenderData,final double distanceSq,boolean resortOnly,Profiler profiler) {
    ...
    if(MinecraftClient.getInstance().isOnThread()) {
       ...
       * () -> uploadChunkBlocks(layer, allocators, renderChunk, chunkRenderData, distanceSq,
       * resortOnly),
       * null);
       */
      ...
      ListenableFutureTask<Object> futureTask=ListenableFutureTask.<Object>create(new Runnable() {
        @Override
        public void run() {
          ChunkRenderDispatcherBoxmatica.this.uploadChunkBlocks(layer,allocators,renderChunk,chunkRenderData,distanceSq,resortOnly,profiler);
        }
      },null);

      synchronized(this.queueChunkUploads) {
        this.queueChunkUploads.add(new ChunkRenderDispatcherBoxmatica.PendingUpload(futureTask,distanceSq));
        profiler.pop();
        return futureTask;
      }
    }
  ...
  protected ListenableFuture<Object> uploadChunkOverlay(final OverlayRenderType type,final BufferAllocatorCache allocators,final ChunkRendererSchematicVbo renderChunk,final ChunkRenderDataSchematic compiledChunk,final double distanceSq,boolean resortOnly,Profiler profiler) {
    ...
    if(MinecraftClient.getInstance().isOnThread()) {
      ...
      ListenableFutureTask<Object> futureTask=ListenableFutureTask.<Object>create(new Runnable() {
        @Override
        public void run() {
          ChunkRenderDispatcherBoxmatica.this.uploadChunkOverlay(type,allocators,renderChunk,compiledChunk,distanceSq,resortOnly,profiler);
        }
      },null);
  ...
  protected void stopWorkerThreads() {
    ...
    for(ChunkRenderWorkerBoxmatica worker:this.listThreadedWorkers) {
      worker.notifyToStop();
    }
  ...
  protected static class PendingUpload implements Comparable<ChunkRenderDispatcherBoxmatica.PendingUpload>{
    ...
    public int compareTo(ChunkRenderDispatcherBoxmatica.PendingUpload other) {
