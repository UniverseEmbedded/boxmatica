...
public class ChunkRenderWorkerBoxmatica implements Runnable{
  private static final Logger LOGGER=Boxmatica.LOGGER;

  private final ChunkRenderDispatcherBoxmatica chunkRenderDispatcher;
  final private BufferAllocatorCache allocatorCache;
  private boolean shouldRun;
  private Profiler profiler;

  public ChunkRenderWorkerBoxmatica(ChunkRenderDispatcherBoxmatica chunkRenderDispatcherIn,Profiler profiler) {
    this(chunkRenderDispatcherIn,null,profiler);
  }

  public ChunkRenderWorkerBoxmatica(ChunkRenderDispatcherBoxmatica chunkRenderDispatcherIn,@Nullable BufferAllocatorCache allocatorCache,Profiler profiler) {
    this.shouldRun=true;
   ...
   * oldStatus = task.casStatus(ChunkRenderTaskSchematic.Status.PENDING,
   * ChunkRenderTaskSchematic.Status.COMPILING);
   * 
   ...
   * }
   * 
   * oldStatus = task.casStatus(ChunkRenderTaskSchematic.Status.COMPILING,
   * ChunkRenderTaskSchematic.Status.UPLOADING);
   ...
   * futuresList.add(this.chunkRenderDispatcher.uploadChunkBlocks(layer, allocators, renderChunk,
   * chunkRenderData, task.getDistanceSq(), false));
   ...
   * if (!chunkRenderData.isBlockLayerEmpty(layer))
   * futuresList.add(this.chunkRenderDispatcher.uploadChunkBlocks(RenderLayer.getTranslucent(),
   * allocators, renderChunk, chunkRenderData, task.getDistanceSq(), true));
   * if (!chunkRenderData.isOverlayTypeEmpty(OverlayRenderType.QUAD))
   * futuresList.add(this.chunkRenderDispatcher.uploadChunkOverlay(OverlayRenderType.QUAD,
   * allocators, renderChunk, chunkRenderData, task.getDistanceSq(), true));
   * }
   * }
   * 
   ...
   * {
   * ChunkRenderWorkerBoxmatica.this.clearRenderAllocators(task);
   ...
   * {
   * ChunkRenderWorkerBoxmatica.this.resetRenderAllocators(task);
   * if (!(throwable instanceof CancellationException) && !(throwable instanceof
   * InterruptedException))
   * MinecraftClient.getInstance().setCrashReportSupplier(CrashReport.create(throwable,
   * "Rendering Boxmatica chunk"));
   * }
      ...
      Futures.addCallback(listenablefuture,new FutureCallback<>() {
        ...
        public void onFailure(@NotNull Throwable throwable) {
          ...
          if((throwable instanceof CancellationException)==false&&(throwable instanceof InterruptedException)==false) {
            MinecraftClient.getInstance().setCrashReportSupplier(CrashReport.create(throwable,"Rendering Boxmatica chunk"));
          }
        }
      },MoreExecutors.directExecutor());
    }
