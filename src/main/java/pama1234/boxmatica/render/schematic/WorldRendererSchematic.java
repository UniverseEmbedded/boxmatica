...
import pama1234.boxmatica.world.ChunkSchematic;
...
public class WorldRendererSchematic{
  ...
  private ChunkRenderDispatcherBoxmatica renderDispatcher;
  ...
  protected ChunkRenderDispatcherBoxmatica getRenderDispatcher() {
  ...
  public void loadRenderers(@Nullable Profiler profiler) {
    if(this.hasWorld()) {
      ...
      if(this.renderDispatcher==null) {
        this.renderDispatcher=new ChunkRenderDispatcherBoxmatica(profiler);
      }
  ...
  public void setupTerrain(Camera camera,Frustum frustum,int frameCount,boolean playerSpectator,Profiler profiler) {
    ...
    if(this.displayListEntitiesDirty) {
      ...
      for(ChunkPos chunkPos:positions) {
        ...
        int cx=chunkPos.x;
        int cz=chunkPos.z;
        //Boxmatica.logger.warn("setupTerrain() [WorldRenderer] positions[{}] chunkPos: {} // isLoaded: {}", i, chunkPos.toString(), this.world.getChunkProvider().isChunkLoaded(cx, cz));
    ...
    for(int i=startIndex;i!=stopIndex;i+=increment) {
      ...
      if(!renderer.getChunkRenderData().isBlockLayerEmpty(renderLayer)) {
        ...
        if(buffers==null||buffers.isClosed()||!renderer.getChunkRenderData().getBuiltBufferCache().hasBuiltBufferByLayer(renderLayer)) {
          //                    Boxmatica.LOGGER.error("Layer [{}], ChunkOrigin [{}], NO BUFFERS!", ChunkRenderLayers.getFriendlyName(renderLayer), chunkOrigin.toShortString());
          continue;
        }

        //                if (buffers == null)
        //                {
        //                    Boxmatica.LOGGER.error("Layer [{}], ChunkOrigin [{}], NO BUFFERS (NULL)", ChunkRenderLayers.getFriendlyName(renderLayer), chunkOrigin.toShortString());
        //                    continue;
        //                }
        //
        //                if (buffers.isClosed())
        //                {
        //                    Boxmatica.LOGGER.error("Layer [{}], ChunkOrigin [{}], NO BUFFERS (CLOSED)", ChunkRenderLayers.getFriendlyName(renderLayer), chunkOrigin.toShortString());
        //                    continue;
        //                }
        //
        //                if (!renderer.getChunkRenderData().getBuiltBufferCache().hasBuiltBufferByLayer(renderLayer))
        //                {
        //                    Boxmatica.LOGGER.error("Layer [{}], ChunkOrigin [{}], NO BUFFERS (NO DATA) ", ChunkRenderLayers.getFriendlyName(renderLayer), chunkOrigin.toShortString());
        //                    continue;
        //                }
         ...
         * {
         * chunkOffsetUniform.set((float)(chunkOrigin.getX() - x), (float)(chunkOrigin.getY() - y),
         * (float)(chunkOrigin.getZ() - z));
         * chunkOffsetUniform.upload();
         * }
         */

        arrayList.add(new RenderPass.RenderObject(0,buffers.getVertexBuffer(),indexBuffer,indexType,0,buffers.getIndexCount(),
      ...
      if(renderer.getChunkRenderData()!=ChunkRenderDataSchematic.EMPTY&&renderer.hasOverlay()) {
        ...
        if(!compiledChunk.isOverlayTypeEmpty(type)) {
          ...
          if(buffers==null||buffers.isClosed()||!renderer.getChunkRenderData().getBuiltBufferCache().hasBuiltBufferByType(type)) {
            //                        Boxmatica.LOGGER.error("Overlay [{}], ChunkOrigin [{}], NO BUFFERS", type.name(), chunkOrigin.toShortString());
            continue;
          }

          //                    if (buffers == null)
          //                    {
          //                        Boxmatica.LOGGER.error("Overlay [{}], ChunkOrigin [{}], NO BUFFERS (NULL)", type.name(), chunkOrigin.toShortString());
          //                        continue;
          //                    }
          //
          //                    if (buffers.isClosed())
          //                    {
          //                        Boxmatica.LOGGER.error("Overlay [{}], ChunkOrigin [{}], NO BUFFERS (CLOSED)", type.name(), chunkOrigin.toShortString());
          //                        continue;
          //                    }
          //
          //                    if (!renderer.getChunkRenderData().getBuiltBufferCache().hasBuiltBufferByType(type))
          //                    {
          //                        Boxmatica.LOGGER.error("Overlay [{}], ChunkOrigin [{}], NO BUFFERS (NO DATA)", type.name(), chunkOrigin.toShortString());
          //                        continue;
          //                    }
  ...
  private void dumpTexture(@Nonnull GpuTexture gpuTexture,Identifier id) {
    ...
    try(GpuBuffer.ReadView readView=RenderSystem.getDevice().createCommandEncoder().readBuffer(gpuBuffer)) {
      ...
      try {
        ...
        Boxmatica.LOGGER.error("dumpTexture: Error saving debug texture for [{}]",id.toString());
      }
  ...
  public boolean renderBlock(BlockRenderView world,BlockState state,BlockPos pos,MatrixStack matrixStack,BufferBuilder bufferBuilderIn) {
    ...
    try {
      ...
      if(renderType==BlockRenderType.INVISIBLE) {
         ...
         * this.blockRenderManager.renderBlock(state, pos, world, matrixStack, bufferBuilderIn, true,
  ...
  private void drawInternal(@Nullable Framebuffer otherFb,RenderPipeline pipeline,
    ...
    if(RenderSystem.isOnRenderThread()) {
      ...
      //            Boxmatica.LOGGER.warn("WorldRendererSchematic#drawInternal() [{}] --> setup IndexBuffer", buffers.getName());
      RenderSystem.ShapeIndexBuffer shapeIndexBuffer=RenderSystem.getSequentialBuffer(pipeline.getVertexFormatMode());
      GpuBuffer indexBuffer;
      VertexFormat.IndexType indexType;
      ...
      if(buffers.getIndexBuffer()==null) {
        if(buffers.getIndexCount()>0) {
          ...
          Boxmatica.LOGGER.error("WorldRendererSchematic#drawInternal() [{}] --> setup IndexBuffer --> NO INDEX COUNT!",buffers.getName());
          return;
        }
      }else {
        indexBuffer=buffers.getIndexBuffer();
      ...
      //            Boxmatica.LOGGER.warn("WorldRendererSchematic#drawInternal() [{}] --> new renderPass", buffers.getName());
      ...
      try(RenderPass pass=RenderSystem.getDevice()
        ...
        //                Boxmatica.LOGGER.warn("WorldRendererSchematic#drawInternal() [{}] renderPass --> setPipeline() [{}]", buffers.getName(), pipeline.getLocation().toString());
        pass.setPipeline(pipeline);

        //                if (this.defaultTexId > -1 && this.defaultTexId < 12 && this.defaultTex != null)
        //                {
        ////                    Boxmatica.LOGGER.warn("WorldRendererSchematic#drawInternal() [{}] renderPass --> bindSampler() [{}] // DEFAULT", buffers.getName(), this.defaultTexId);
        ...
        //                    {
        //                        Boxmatica.LOGGER.warn("WorldRendererSchematic#drawInternal() [{}] renderPass --> bindSampler() [{}] // OTHER", buffers.getName(), i);
        ...
        if(setLineWidth) {
          ...
          //                    Boxmatica.LOGGER.warn("WorldRendererSchematic#drawInternal() [{}] renderPass --> setUniform() // lineWidth [{}]", buffers.getName(), width);
          pass.setUniform("LineWidth",width);
        }

        //                Boxmatica.LOGGER.warn("WorldRendererSchematic#drawInternal() [{}] renderPass --> setVertexBuffer() [0]", buffers.getName());
        pass.setVertexBuffer(0,buffers.getVertexBuffer());
        //                Boxmatica.LOGGER.warn("WorldRendererSchematic#drawInternal() [{}] renderPass --> setIndexBuffer() [{}]", buffers.getName(), indexType.name());
        pass.setIndexBuffer(indexBuffer,indexType);
        //                Boxmatica.LOGGER.warn("WorldRendererSchematic#drawInternal() [{}] renderPass --> drawIndexed() [0, {}]", buffers.getName(), buffers.getIndexCount());
        pass.drawIndexed(0,buffers.getIndexCount());
      }

      //            Boxmatica.LOGGER.warn("WorldRendererSchematic#drawInternal() [{}] --> END", buffers.getName());
  ...
  public List<BlockModelPart> getModelParts(BlockPos pos,BlockState state,Random rand) {
    ...
    if(parts.isEmpty()) {
      ...
      Boxmatica.LOGGER.warn("getModelParts: Invalid Block State for block at [{}] with state [{}]; Resetting to default.",pos.toShortString(),state.toString());
    }
  ...
  public void renderEntities(Camera camera,Frustum frustum,MatrixStack matrices,VertexConsumerProvider.Immediate immediate,float partialTicks,Profiler profiler) {
    ...
    if(this.renderEntitiesStartupCounter>0) {
      ...
      for(ChunkRendererSchematicVbo chunkRenderer:this.renderInfos) {
        ...
        //                Boxmatica.LOGGER.error("[WorldRenderer] Chunk: [{}], EntityList [{}]", pos.toShortString(), list.size());
        ...
        for(Entity entityTmp:list) {
          ...
          if(shouldRender) {
            ...
            //                        Boxmatica.LOGGER.warn("[WorldRenderer] Chunk: [{}], EntityPos [{}] // Adj. Pos: X [{}], Y [{}], Z [{}]", pos.toShortString(), entityTmp.getBlockPos().toShortString(), x, y, z);
          ...
          //                    else
          //                    {
          //                        Boxmatica.LOGGER.warn("Skipping Entity at pos X: [{}], Y: [{}], Z: [{}] (Should Render = False)", entityTmp.getX(), entityTmp.getY(), entityTmp.getZ());
          //                    }
        }
      }
    }
        ...
        if(chunk!=null&&data.getTimeBuilt()>=chunk.getTimeCreated()) {
          for(BlockEntity te:tiles) {
            ...
            try {
              ...
              Boxmatica.LOGGER.error("[Pass 1] Error rendering blockEntities; Exception: {}",err.getLocalizedMessage());
            }
          }
        }
      }
    ...
    synchronized(this.blockEntities) {
      for(BlockEntity te:this.blockEntities) {
        ...
        try {
          ...
          Boxmatica.LOGGER.error("[Pass 2] Error rendering blockEntities; Exception: {}",err.getLocalizedMessage());
        }
      }
    }
   ...
   * return true;
   * }
   * else if (this.mc.player.isSpectator() && this.mc.options.keySpectatorOutlines.isPressed() &&
   * entityIn instanceof PlayerEntity)
   * {
   * return entityIn.ignoreFrustumCheck ||
   * camera.isBoundingBoxInFrustum(entityIn.getBoundingBox()) ||
