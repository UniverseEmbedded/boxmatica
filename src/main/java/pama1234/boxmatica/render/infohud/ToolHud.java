package pama1234.boxmatica.render.infohud;
...
import pama1234.boxmatica.schematic.projects.SchematicProject;
import pama1234.boxmatica.schematic.projects.SchematicVersion;
...
import pama1234.boxmatica.selection.SelectionMode;
import pama1234.boxmatica.tool.ToolMode;
import pama1234.boxmatica.tool.ToolModeData;
import pama1234.boxmatica.util.EntityUtils;
import pama1234.boxmatica.util.PasteLayerBehavior;
import pama1234.boxmatica.util.PositionUtils;
import pama1234.boxmatica.util.ReplaceBehavior;
...
public class ToolHud extends InfoHud{
  ...
  protected void updateHudText() {
    ...
    if(hasTool&&DataManager.getSchematicProjectsManager().hasProjectOpen()) {
      ...
      lines.add(StringUtils.translate("boxmatica.hud.schematic_projects.project_name",green+project.getName()+rst));
      SchematicVersion version=project.getCurrentVersion();

      if(version!=null) {
        lines.add(StringUtils.translate("boxmatica.hud.schematic_projects.current_version",green+version.getVersion()+rst,green+project.getVersionCount()+rst,green+version.getName()+rst));
        DATE.setTime(version.getTimeStamp());
        lines.add(StringUtils.translate("boxmatica.hud.schematic_projects.current_version_date",green+SIMPLE_DATE_FORMAT.format(DATE)+rst));
        BlockPos o=project.getOrigin();
        str=String.format("%d, %d, %d",o.getX(),o.getY(),o.getZ());
        lines.add(StringUtils.translate("boxmatica.hud.schematic_projects.origin",green+str+rst));
      }else {
        lines.add(StringUtils.translate("boxmatica.hud.schematic_projects.no_versions"));
      }
      ...
      if(selection!=null&&sm.getSelectionMode()==SelectionMode.NORMAL) {
        ...
        if(subRegionName!=null) {
          lines.add(StringUtils.translate("boxmatica.hud.area_selection.selected_sub_region",green+subRegionName+rst));
        }
      }

      str=green+Configs.Generic.SELECTION_CORNERS_MODE.getOptionListValue().getDisplayName()+rst;
      lines.add(StringUtils.translate("boxmatica.hud.area_selection.selection_corners_mode",str));
      ...
      if(StatusInfoRenderer.getInstance().shouldRenderStatusInfoHud()==false) {
        lines.add(StringUtils.translate("boxmatica.hud.schematic_projects_mode"));
      }
    ...
    String strYes=green+StringUtils.translate("boxmatica.label.yes")+rst;
    String strNo=GuiBase.TXT_RED+StringUtils.translate("boxmatica.label.no")+rst;

    if(hasTool&&mode==ToolMode.DELETE) {
      String strp=ToolModeData.DELETE.getUsePlacement()?"boxmatica.hud.delete.target_mode.placement":"boxmatica.hud.delete.target_mode.area";
      lines.add(StringUtils.translate("boxmatica.hud.delete.target_mode",green+StringUtils.translate(strp)+rst));
    }
    ...
    if(hasTool&&mode.getUsesAreaSelection()) {
      ...
      if(selection!=null) {
        ...
        if(sm.getSelectionMode()==SelectionMode.NORMAL) {
          lines.add(StringUtils.translate("boxmatica.hud.area_selection.selected_area_normal",name));
        }else {
          lines.add(StringUtils.translate("boxmatica.hud.area_selection.selected_area_simple",name));
        }
        ...
        if(o==null) {
          ...
          strOr=StringUtils.translate("boxmatica.gui.label.origin.auto");
        }else {
          strOr=StringUtils.translate("boxmatica.gui.label.origin.manual");
        }
        int count=selection.getAllSubRegionBoxes().size();

        str=String.format("%d, %d, %d %s[%s%s%s]",o.getX(),o.getY(),o.getZ(),rst,orange,strOr,rst);
        String strOrigin=StringUtils.translate("boxmatica.hud.area_selection.origin",green+str+rst);
        String strBoxes=StringUtils.translate("boxmatica.hud.area_selection.box_count",green+count+rst);
        ...
        if(subRegionName!=null&&box!=null) {
          ...
          if(p1!=null&&p2!=null) {
            ...
            lines.add(StringUtils.translate("boxmatica.hud.area_selection.dimensions_position",strDim,strp1,strp2));
          }
        }
      }
      ...
      if(mode.getUsesBlockPrimary()) {
        ...
        if(state!=null) {
          lines.add(StringUtils.translate("boxmatica.tool_hud.block_1",this.getBlockString(state)));
        }
      }
      ...
      if(mode.getUsesBlockSecondary()) {
        ...
        if(state!=null) {
          lines.add(StringUtils.translate("boxmatica.tool_hud.block_2",this.getBlockString(state)));
        }
      }
      ...
      if(schematicPlacement!=null) {
        str=StringUtils.translate("boxmatica.hud.schematic_placement.selected_placement");
        lines.add(String.format("%s: %s%s%s",str,green,schematicPlacement.getName(),rst));

        str=StringUtils.translate("boxmatica.hud.schematic_placement.sub_region_count");
        int count=schematicPlacement.getSubRegionCount();
        String strCount=String.format("%s: %s%d%s",str,green,count,rst);

        str=StringUtils.translate("boxmatica.hud.schematic_placement.sub_regions_modified");
        ...
        lines.add(StringUtils.translate("boxmatica.hud.area_selection.origin",green+str+rst));
        ...
        if(placement!=null) {
          ...
          str=StringUtils.translate("boxmatica.hud.schematic_placement.selected_sub_region");
          String str2=StringUtils.translate("boxmatica.hud.schematic_placement.sub_region_modified");
          ...
          str=String.format("%d, %d, %d",or.getX(),or.getY(),or.getZ());
          lines.add(StringUtils.translate("boxmatica.hud.schematic_placement.sub_region_origin",green+str+rst));
        }
        ...
        if(mode==ToolMode.PASTE_SCHEMATIC) {
          ...
          lines.add(StringUtils.translate("boxmatica.hud.misc.schematic_paste.replace_mode",str));
          ...
          lines.add(StringUtils.translate("boxmatica.hud.misc.schematic_paste.layer_mode",str));
          ...
          lines.add(StringUtils.translate("boxmatica.hud.misc.schematic_paste.data_restore_mode",str));

          String strVal=Configs.Generic.PASTE_IGNORE_INVENTORY.getBooleanValue()?strYes:strNo;
          lines.add(StringUtils.translate("boxmatica.hud.misc.schematic_paste.ignore_inventory_contents",strVal));
        }
      }else {
        String strTmp="<"+StringUtils.translate("boxmatica.label.none_lower")+">";
        str=StringUtils.translate("boxmatica.hud.schematic_placement.selected_placement");
    ...
    if(hasTool||mode==ToolMode.REBUILD) {
      str=StringUtils.translate("boxmatica.hud.selected_mode");
  ...
  protected String getBlockString(BlockState state) {
    ...
    if(facing.isPresent()) {
      ...
      strBlock+=" - "+StringUtils.translate("boxmatica.tool_hud.facing",strFacing);
    }
