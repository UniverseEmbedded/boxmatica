...
public class StatusInfoRenderer implements IInfoHudRenderer{
  ...
  public List<String> getText(RenderPhase phase) {
    ...
    if(Configs.Generic.EASY_PLACE_MODE.getBooleanValue()) {
      lines.add(StringUtils.translate("boxmatica.hud.misc.easy_place_mode_enabled"));
    }else if(Configs.Generic.PLACEMENT_RESTRICTION.getBooleanValue()) {
      lines.add(StringUtils.translate("boxmatica.hud.misc.placement_restriction_mode_enabled"));
    }
    ...
    if(range.getLayerMode()==LayerMode.ALL) {
      lines.add(StringUtils.translate("boxmatica.hud.misc.render_layer_mode_all",g+strMode+rst));
    }else {
      String strVal=String.format("%s%s = %s%s",g,axisName,val,rst);
      lines.add(StringUtils.translate("boxmatica.hud.misc.render_layer_mode",g+strMode+rst,g+strVal+rst));
    }

    String strOn=g+StringUtils.translate("boxmatica.message.value.on")+rst;
    String strOff=red+StringUtils.translate("boxmatica.message.value.off")+rst;
    ...
    lines.add(StringUtils.translate("boxmatica.hud.misc.renderer_status",strAll,strSch,strBlk,strOvl,strSel));
