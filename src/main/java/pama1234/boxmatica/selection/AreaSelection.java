...
public class AreaSelection{
  ...
  /**
   * Adds the given SelectionBox, if either replace is true, or there isn't yet a box by the same
   * name.
   * 
   * @param box
   * @param replace
   * @return true if the box was successfully added, false if replace was false and there was
   *         already a box with the same name
   */
  ...
  public boolean renameSubRegionBox(String oldName,String newName,@Nullable IMessageConsumer feedback) {
    ...
    if(box!=null) {
      if(this.subRegionBoxes.containsKey(newName)) {
        if(feedback!=null) {
          feedback.addMessage(MessageType.ERROR,"boxmatica.error.area_editor.rename_sub_region.exists",newName);
        }
  ...
  public void moveEntireSelectionTo(BlockPos newOrigin,boolean printMessage) {
    ...
    if(printMessage) {
      ...
      InfoUtils.showGuiOrActionBarMessage(MessageType.SUCCESS,"boxmatica.message.moved_selection",oldStr,newStr);
    }
  }
