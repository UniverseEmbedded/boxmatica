package pama1234.boxmatica.selection;
...
import pama1234.boxmatica.gui.GuiAreaSelectionEditorNormal;
import pama1234.boxmatica.gui.GuiAreaSelectionEditorSimple;
...
import pama1234.boxmatica.util.RayTraceUtils;
...
public class SelectionManager{
  ...
  public void switchSelectionMode() {
    if(DataManager.getSchematicProjectsManager().hasProjectOpen()) {
      ...
      if(project!=null) {
        ...
        InfoUtils.showGuiOrInGameMessage(MessageType.WARNING,"boxmatica.error.schematic_projects.in_projects_mode_but_no_project_open");
      }
    }else {
  ...
  public boolean renameSelection(Path dir,String selectionId,String newName,boolean copy,IMessageConsumer feedback) {
    ...
    if(Files.exists(file)&&Files.isRegularFile(file)) {
      ...
      if(newFileName.isEmpty()) {
        feedback.addMessage(MessageType.ERROR,"boxmatica.error.area_selection.rename.invalid_safe_file_name",newFileName);
        return false;
      }
      ...
      if(!Files.exists(newFile)&&(copy||FileUtils.move(file,newFile))) {
        ...
        if(copy) {
          try {
            ...
            feedback.addMessage(MessageType.ERROR,"boxmatica.error.area_selection.copy_failed");
            Boxmatica.LOGGER.warn("Copy failed",e);
            return false;
          }
        ...
        feedback.addMessage(MessageType.ERROR,"boxmatica.error.area_selection.rename.already_exists",newFile.getFileName());
      }
    }
    ...
    if(selection!=null&&mc.player!=null) {
      ...
      if(selection.createNewSubRegionBox(pos,selection.getName())!=null) {
        if(printMessage) {
          ...
          InfoUtils.showGuiOrActionBarMessage(MessageType.SUCCESS,"boxmatica.message.added_selection_box",posStr);
        }
  ...
  public boolean createNewSubRegionIfDoesntExist(String name,MinecraftClient mc,IMessageConsumer feedback) {
    ...
    if(selection!=null&&mc.player!=null) {
      if(selection.getSubRegionBox(name)!=null) {
        feedback.addMessage(MessageType.ERROR,"boxmatica.error.area_editor.create_sub_region.exists",name);
        return false;
      }
      ...
      if(selection.createNewSubRegionBox(pos,name)!=null) {
        String posStr=String.format("x: %d, y: %d, z: %d",pos.getX(),pos.getY(),pos.getZ());
        feedback.addMessage(MessageType.SUCCESS,"boxmatica.message.added_selection_box",posStr);
  ...
  public boolean createSelectionFromPlacement(Path dir,SchematicPlacement placement,String name,IMessageConsumer feedback) {
    ...
    if(safeName.isEmpty()) {
      feedback.addMessage(MessageType.ERROR,"boxmatica.error.area_selection.rename.invalid_safe_file_name",safeName);
      return false;
    }
    ...
    feedback.addMessage(MessageType.ERROR,"boxmatica.error.area_selection.create_failed",safeName);
  ...
  public boolean grabElement(MinecraftClient mc,int maxDistance) {
    ...
    if(area!=null&&entity!=null&&area.getAllSubRegionBoxes().size()>0) {
      ...
      if(trace.getHitType()==HitType.SELECTION_BOX_CORNER||trace.getHitType()==HitType.SELECTION_BOX_BODY) {
        ...
        InfoUtils.printActionbarMessage("boxmatica.message.grabbed_element_for_moving");
        return true;
      }
    }
    ...
    if(area!=null) {
      ...
      if(movingCorner||movingOrigin) {
        ...
        // Moving a corner
          ...
          String posStr=String.format("x: %d, y: %d, z: %d",pos.getX(),pos.getY(),pos.getZ());
          InfoUtils.printActionbarMessage("boxmatica.message.set_selection_box_point",cornerIndex,posStr);
        }
      }
    }
  }
  ...
  public void moveSelectionOrigin(AreaSelection area,BlockPos newOrigin,boolean moveEntireSelection) {
    if(moveEntireSelection) {
      ...
      InfoUtils.showGuiOrActionBarMessage(MessageType.SUCCESS,"boxmatica.message.moved_area_origin",posStrOld,posStrNew);
    }
  }
  ...
  public GuiBase getEditGui() {
    ...
    if(this.getSelectionMode()==SelectionMode.NORMAL) {
      if(selection!=null) {
        ...
        InfoUtils.showGuiOrActionBarMessage(MessageType.WARNING,"boxmatica.error.area_editor.open_gui.no_selection");
        return null;
      }
    }else {
  ...
  public JsonObject toJson() {
    ...
    try {
      ...
      Boxmatica.LOGGER.warn("Exception while writing area selections to disk",e);
    }
