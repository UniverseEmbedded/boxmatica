...
import pama1234.boxmatica.gui.widgets.WidgetListSelectionSubRegions;
import pama1234.boxmatica.gui.widgets.WidgetSelectionSubRegion;
import pama1234.boxmatica.materials.MaterialListAreaAnalyzer;
...
import pama1234.boxmatica.util.SchematicUtils;
...
public class GuiAreaSelectionEditorNormal extends GuiListBase<String,WidgetSelectionSubRegion,WidgetListSelectionSubRegions>
  ...
  public GuiAreaSelectionEditorNormal(AreaSelection selection) {
    ...
    if(DataManager.getSchematicProjectsManager().hasProjectOpen()) {
      this.title=StringUtils.translate("boxmatica.gui.title.area_editor_normal_schematic_projects");
    }else {
      this.title=StringUtils.translate("boxmatica.gui.title.area_editor_normal");
    }
  }
  ...
  public void initGui() {
    ...
    if(this.selection!=null) {
      ...
      this.addLabel(20,30,120,12,0xFFFFAA00,StringUtils.translate("boxmatica.error.area_editor.no_selection"));
    }
  }
  ...
  protected void createSelectionEditFields() {
    ...
    this.addLabel(x,y,-1,16,0xFFFFFFFF,StringUtils.translate("boxmatica.gui.label.area_editor.selection_name"));
    y+=13;
  ...
  protected int addSubRegionFields(int x,int y) {
    ...
    this.addLabel(x,y,-1,16,0xFFFFFFFF,GuiBase.TXT_BOLD+StringUtils.translate("boxmatica.gui.label.area_editor.sub_regions",str));
    ...
    if(DataManager.getSchematicProjectsManager().hasProjectOpen()) {
      ...
      button.setHoverStrings("boxmatica.gui.button.hover.schematic_projects.area_browser_disabled_currently_in_projects_mode");
    }
  ...
  protected void addRenderingDisabledWarning(int x,int y) {
    if(Configs.Visuals.ENABLE_AREA_SELECTION_RENDERING.getBooleanValue()==false) {
      ...
      String str=StringUtils.translate("boxmatica.warning.area_editor.area_rendering_disabled",configName,hotkeyName,hotkeyVal);
  ...
  protected int createCoordinateInputs(int x,int y,int width,Corner corner) {
    ...
    switch(corner) {
      case CORNER_1:
        label=StringUtils.translate("boxmatica.gui.label.area_editor.corner_1");
        widget=new WidgetCheckBox(x,y+3,Icons.CHECKBOX_UNSELECTED,Icons.CHECKBOX_SELECTED,label);
        this.checkBoxCorner1=widget;
        break;
      case CORNER_2:
        label=StringUtils.translate("boxmatica.gui.label.area_editor.corner_2");
        widget=new WidgetCheckBox(x,y+3,Icons.CHECKBOX_UNSELECTED,Icons.CHECKBOX_SELECTED,label);
        this.checkBoxCorner2=widget;
        break;
      case NONE:
        label=StringUtils.translate("boxmatica.gui.label.area_editor.origin");
        widget=new WidgetCheckBox(x,y+3,Icons.CHECKBOX_UNSELECTED,Icons.CHECKBOX_SELECTED,label);
  ...
  protected int createButton(int x,int y,int width,@Nullable Corner corner,ButtonListener.Type type) {
    ...
    if(type==ButtonListener.Type.CHANGE_SELECTION_MODE) {
      ...
      label=StringUtils.translate("boxmatica.gui.button.save_new_schematic_version");
    }else {
    ...
    if(type==ButtonListener.Type.CREATE_SCHEMATIC&&projectsMode==false) {
      button.setHoverStrings("boxmatica.gui.button.hover.area_editor.shift_for_in_memory");
    }
  ...
  protected void createCoordinateButton(int x,int y,Corner corner,CoordinateType coordType,ButtonListener.Type type) {
    String hover=StringUtils.translate("boxmatica.gui.button.hover.plus_minus_tip_ctrl_alt_shift");
  ...
  protected static class ButtonListener implements IButtonActionListener{
      ...
      switch(this.type) {
        ...
        case CHANGE_SELECTION_MODE:
          ...
          if(newMode==SelectionMode.NORMAL&&manager.hasNormalSelection()==false) {
            this.parent.addMessage(MessageType.WARNING,"boxmatica.error.area_editor.switch_mode.no_selection");
          }else {
        ...
        case CREATE_SUB_REGION: {
          GuiTextInput gui=new GuiTextInput(512,"boxmatica.gui.title.area_editor.sub_region_name","",null,new SubRegionCreator(this.parent));
    ...
    public enum Type{
      SET_SELECTION_NAME("boxmatica.gui.button.area_editor.set_selection_name"),
      SET_BOX_NAME("boxmatica.gui.button.area_editor.set_box_name"),
      TOGGLE_ORIGIN_ENABLED("boxmatica.gui.button.area_editor.origin_enabled"),
      CREATE_SUB_REGION("boxmatica.gui.button.area_editor.create_sub_region"),
      CREATE_SCHEMATIC("boxmatica.gui.button.area_editor.create_schematic"),
      ANALYZE_AREA("boxmatica.gui.button.area_editor.analyze_area"),
      CHANGE_SELECTION_MODE("boxmatica.gui.button.area_editor.change_selection_mode"),
      CHANGE_CORNER_MODE("boxmatica.gui.button.area_editor.change_corner_mode"),
      MOVE_TO_PLAYER("boxmatica.gui.button.move_to_player"),
      NUDGE_COORD_X(""),
      NUDGE_COORD_Y(""),
      NUDGE_COORD_Z("");
