...
import pama1234.boxmatica.materials.MaterialListSchematic;
...
public class GuiSchematicLoad extends GuiSchematicBrowserBase{
  public GuiSchematicLoad() {
    ...
    this.title=StringUtils.translate("boxmatica.gui.title.load_schematic");
  }
  ...
  public void initGui() {
    ...
    label=StringUtils.translate("boxmatica.gui.label.schematic_load.checkbox.create_placement");
    String hover=StringUtils.translate("boxmatica.gui.label.schematic_load.hoverinfo.create_placement");
  ...
  private int createButton(int x,int y,int width,ButtonListener.Type type) {
    ...
    if(type==ButtonListener.Type.MATERIAL_LIST) {
      button.setHoverStrings(StringUtils.translate("boxmatica.gui.button.hover.material_list_shift_to_select_sub_regions"));
    }
  ...
  private static class ButtonListener implements IButtonActionListener{
    ...
    public void actionPerformedWithButton(ButtonBase button,int mouseButton) {
      ...
      if(fileType==FileType.LITEMATICA_SCHEMATIC) {
        ...
        schematic=WorldUtils.convertStructureToBoxmaticaSchematic(entry.getDirectory(),entry.getName());
        warnType=true;
      }else if(fileType==FileType.SPONGE_SCHEMATIC) {
        schematic=WorldUtils.convertSpongeSchematicToBoxmaticaSchematic(entry.getDirectory(),entry.getName());
        warnType=true;
      }else {
        this.gui.addMessage(MessageType.ERROR,"boxmatica.error.schematic_load.unsupported_type",file.getFileName());
      }

      if(schematic!=null) {
        if(this.type==Type.LOAD_SCHEMATIC) {
          SchematicHolder.getInstance().addSchematic(schematic,true);
          this.gui.addMessage(MessageType.SUCCESS,"boxmatica.info.schematic_load.schematic_loaded",file.getFileName());
          ...
          if(GuiBase.isShiftDown()) {
            ...
            gui.setTitle(StringUtils.translate("boxmatica.gui.title.material_list.select_schematic_regions",schematic.getMetadata().getName()));
        ...
        if(warnType) {
          InfoUtils.showGuiOrInGameMessage(MessageType.WARNING,15000,"boxmatica.message.warn.schematic_load_non_boxmatica");
        }
      }
    }

    public enum Type{
      LOAD_SCHEMATIC("boxmatica.gui.button.load_schematic_to_memory"),
      MATERIAL_LIST("boxmatica.gui.button.material_list");
  ...
  private static class MaterialListCreator implements IStringListConsumer{
    ...
    public MaterialListCreator(BoxmaticaSchematic schematic) {
