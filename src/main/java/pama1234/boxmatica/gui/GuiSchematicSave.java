...
public class GuiSchematicSave extends GuiSchematicSaveBase implements ICompletionListener{
  ...
  public GuiSchematicSave(@Nullable BoxmaticaSchematic schematic) {
    super(schematic);

    if(schematic!=null) {
      this.title=StringUtils.translate("boxmatica.gui.title.save_schematic_from_memory");
    }else {
      this.title=StringUtils.translate("boxmatica.gui.title.create_schematic_from_selection");
    }
    ...
    public void actionPerformedWithButton(ButtonBase button,int mouseButton) {
      if(this.type==ButtonType.SAVE) {
        ...
        if(this.gui.schematic!=null) {
          BoxmaticaSchematic schematic=this.gui.schematic;
          ...
          if(area!=null) {
            ...
            if(fileNameTmp.endsWith(BoxmaticaSchematic.FILE_EXTENSION)==false) {
              fileNameTmp+=BoxmaticaSchematic.FILE_EXTENSION;
            }

            if(FileUtils.canWriteToFileAsPath(dir,fileNameTmp,overwrite)==false) {
              this.gui.addMessage(MessageType.ERROR,"boxmatica.error.schematic_write_to_file_failed.exists",fileNameTmp);
              return;
            }
            ...
            BoxmaticaSchematic.SchematicSaveInfo info=new BoxmaticaSchematic.SchematicSaveInfo(visibleOnly,includeSupportBlocks,ignoreEntities,fromSchematicWorld);
            BoxmaticaSchematic schematic=BoxmaticaSchematic.createEmptySchematic(area,author);
            ...
            this.gui.addMessage(MessageType.INFO,"boxmatica.message.schematic_save_task_created");
          }else {
            this.gui.addMessage(MessageType.ERROR,"boxmatica.message.error.schematic_save_no_area_selected");
          }
        }
      }
    }
  ...
  public static class InMemorySchematicCreator implements IStringConsumer{
    ...
    public void setString(String string) {
      BoxmaticaSchematic.SchematicSaveInfo info=new BoxmaticaSchematic.SchematicSaveInfo(false,false); // TODO
      String author=this.mc.player.getName().getString();
      BoxmaticaSchematic schematic=BoxmaticaSchematic.createEmptySchematic(this.area,author);
