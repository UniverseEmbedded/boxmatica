...
import pama1234.boxmatica.gui.widgets.WidgetAreaSelectionBrowser;
...
public class GuiAreaSelectionManager extends GuiListBase<DirectoryEntry,WidgetDirectoryEntry,WidgetAreaSelectionBrowser> implements ISelectionListener<DirectoryEntry>{
  ...
  public GuiAreaSelectionManager() {
    ...
    this.title=StringUtils.translate("boxmatica.gui.title.area_selection_manager");
  ...
  public void initGui() {
    ...
    if(this.selectionManager.getSelectionMode()==SelectionMode.SIMPLE) {
      InfoUtils.showGuiMessage(MessageType.WARNING,"boxmatica.message.warn.area_selection.browser_open_in_simple_mode");
    }
  }
  ...
  protected void reCreateGuiElements() {
    ...
    if(currentSelection!=null) {
      ...
      if(currentSelection.length()>len+1) {
        ...
        String str=StringUtils.translate("boxmatica.gui.label.area_selection_manager.current_selection",currentSelection);
  ...
  private int createButton(int x,int y,ButtonListener.ButtonType type) {
    ...
    if(type==ButtonListener.ButtonType.UNSELECT) {
      button.setHoverStrings("boxmatica.gui.button.hover.area_selections.unselect");
    }
  ...
  private static class ButtonListener implements IButtonActionListener{
    ...
    public void actionPerformedWithButton(ButtonBase button,int mouseButton) {
      if(this.type==ButtonType.NEW_SELECTION) {
        ...
        String title="boxmatica.gui.title.create_area_selection";
        ...
        if(placement!=null) {
          Path dir=this.gui.getListWidget().getCurrentDirectory();
          String title="boxmatica.gui.title.create_area_selection_from_placement";
          ...
          this.gui.addMessage(MessageType.ERROR,"boxmatica.error.area_selection.no_placement_selected");
        }
    ...
    public enum ButtonType{
      NEW_SELECTION("boxmatica.gui.button.area_selections.create_new_selection"),
      FROM_PLACEMENT("boxmatica.gui.button.area_selections.create_selection_from_placement"),
      UNSELECT("boxmatica.gui.button.area_selections.unselect");
  ...
  public static class SelectionCreatorPlacement implements IStringConsumerFeedback{
    ...
    public boolean setString(String name) {
      if(this.gui.getSelectionManager().createSelectionFromPlacement(this.dir,this.placement,name,this.gui)) {
        this.gui.addMessage(MessageType.SUCCESS,"boxmatica.message.area_selections.selection_created_from_placement",name);
        this.gui.getListWidget().refreshEntries();
        return true;
      }
