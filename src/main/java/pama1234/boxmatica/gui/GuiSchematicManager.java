...
public class GuiSchematicManager extends GuiSchematicBrowserBase implements ISelectionListener<DirectoryEntry>{
  ...
  public GuiSchematicManager() {
    ...
    this.title=StringUtils.translate("boxmatica.gui.title.schematic_manager");
  }
  ...
  private static class ButtonListener implements IButtonActionListener{
    ...
    public void actionPerformedWithButton(ButtonBase button,int mouseButton) {
      if(this.type==Type.SET_PREVIEW&&mouseButton==1) {
        if(previewGenerator!=null) {
          previewGenerator=null;
          this.gui.addMessage(MessageType.SUCCESS,"boxmatica.message.schematic_preview_cancelled");
        }
      ...
      if(entry==null) {
        this.gui.addMessage(MessageType.ERROR,"boxmatica.error.schematic_load.no_schematic_selected");
        return;
      }
      ...
      if(!Files.exists(file)||!Files.isRegularFile(file)||!Files.isReadable(file)) {
        this.gui.addMessage(MessageType.ERROR,"boxmatica.error.schematic_load.cant_read_file",file.getFileName());
        return;
      }
      ...
      if(this.type==Type.EXPORT_SCHEMATIC) {
        ...
        if(fileType==FileType.BOXMATICA_SCHEMATIC||
          ...
          this.gui.addMessage(MessageType.ERROR,"boxmatica.error.schematic_manager.schematic_import.unsupported_type",file.getFileName());
        }
      }else if(this.type==Type.RENAME_SCHEMATIC) {
        BoxmaticaSchematic schematic=BoxmaticaSchematic.createFromFile(entry.getDirectory(),entry.getName());
        String oldName=schematic!=null?schematic.getMetadata().getName():"";
        GuiBase.openGui(new GuiTextInputFeedback(256,"boxmatica.gui.title.rename_schematic",oldName,this.gui,new SchematicRenamer(entry.getDirectory(),entry.getName(),this.gui)));
        ...
        InfoUtils.showGuiAndInGameMessage(MessageType.INFO,"boxmatica.info.schematic_manager.preview.set_preview_by_taking_a_screenshot");
      }
    }

    public enum Type{
      IMPORT_SCHEMATIC("boxmatica.gui.button.import"),
      EXPORT_SCHEMATIC("boxmatica.gui.button.schematic_manager.export_as"),
      RENAME_SCHEMATIC("boxmatica.gui.button.rename"),
      DELETE_SCHEMATIC("boxmatica.gui.button.delete"),
      SET_PREVIEW("boxmatica.gui.button.set_preview","boxmatica.info.schematic_manager.preview.right_click_to_cancel"),
      EXPORT_TYPE("");
  ...
  private static class SchematicRenamer implements IStringConsumerFeedback{
    ...
    public boolean setString(String string) {
      BoxmaticaSchematic schematic=BoxmaticaSchematic.createFromFile(this.dir,this.fileName);
      ...
      if(schematic!=null) {
        ...
        this.gui.setString(StringUtils.translate("boxmatica.error.schematic_rename.read_failed"));
      }
  ...
  public static class FileDeleter implements IConfirmationListener{
    ...
    public boolean onActionConfirmed() {
      try {
        ...
        InfoUtils.showGuiOrInGameMessage(MessageType.ERROR,"boxmatica.error.generic.failed_to_delete_file",this.file.toAbsolutePath());
      }
      ...
      if(schematic!=null) {
        try {
          ...
          ScreenshotRecorder.takeScreenshot(mc.getFramebuffer(),(screenshot)-> {
            ...
            InfoUtils.showGuiOrInGameMessage(MessageType.SUCCESS,"boxmatica.info.schematic_manager.preview.success");
          });
        }catch(Exception e) {
          Boxmatica.LOGGER.warn("Exception while creating preview image",e);
        }
      }else {
        InfoUtils.showGuiOrInGameMessage(MessageType.ERROR,"boxmatica.error.schematic_rename.read_failed");
      }
    }
  }
  ...
  public enum ExportType implements IConfigOptionListEntry{
    SCHEMATIC("Schematic"),
    V6_LITEMATIC("V6 (1.20.4) Boxmatic"),
