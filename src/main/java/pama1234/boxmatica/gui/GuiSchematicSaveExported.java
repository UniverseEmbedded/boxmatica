...
import pama1234.boxmatica.gui.GuiSchematicManager.ExportType;
...
public class GuiSchematicSaveExported extends GuiSchematicSaveBase{
  ...
  public GuiSchematicSaveExported(DirectoryEntryType type,Path dirSource,String inputFileName,ExportType exportType) {
    ...
    this.title=StringUtils.translate("boxmatica.gui.title.save_exported_schematic",exportType.getDisplayName(),inputFileName);
  ...
  private static class ButtonListener implements IButtonActionListener{
    ...
    public void actionPerformedWithButton(ButtonBase button,int mouseButton) {
      if(this.type==ButtonType.SAVE) {
        ...
        if(this.gui.type==DirectoryEntryType.FILE) {
          ...
          if(fileType==FileType.BOXMATICA_SCHEMATIC) {
            if(this.gui.exportType==ExportType.V6_LITEMATIC) {
              ...
              if(WorldUtils.convertBoxmaticaSchematicToSchematicaSchematic(inDir,inFile,dir,fileName,ignoreEntities,override,this.gui)) {
                this.gui.addMessage(MessageType.SUCCESS,"boxmatica.message.schematic_exported_as",fileName);
                this.gui.getListWidget().refreshEntries();
              }
            }else if(this.gui.exportType==ExportType.VANILLA) {
              if(WorldUtils.convertBoxmaticaSchematicToVanillaStructure(inDir,inFile,dir,fileName,ignoreEntities,override,this.gui)) {
                this.gui.addMessage(MessageType.SUCCESS,"boxmatica.message.schematic_exported_as",fileName);
                this.gui.getListWidget().refreshEntries();
              }
            }
        ...
        this.gui.addMessage(MessageType.ERROR,"boxmatica.error.schematic_export.unsupported_type",this.gui.inputFileName);
      }
    }
  }
}
