...
import pama1234.boxmatica.gui.widgets.WidgetSchematicProjectBrowser;
...
public class GuiSchematicProjectsBrowser extends GuiListBase<DirectoryEntry,WidgetDirectoryEntry,WidgetSchematicProjectBrowser>
  ...
  public GuiSchematicProjectsBrowser() {
    ...
    this.title=StringUtils.translate("boxmatica.gui.title.schematic_projects_browser");
  }
  ...
  private static class ButtonListener implements IButtonActionListener{
    ...
    public void actionPerformedWithButton(ButtonBase button,int mouseButton) {
      if(this.type==Type.LOAD_PROJECT) {
        ...
        if(entry!=null&&entry.getType()==DirectoryEntryType.FILE) {
          if(DataManager.getSchematicProjectsManager().openProject(entry.getFullPath())) {
            ...
            if(project!=null) {
              ...
              InfoUtils.showGuiOrInGameMessage(MessageType.SUCCESS,"boxmatica.message.schematic_projects.project_loaded",name);
            }else {
              InfoUtils.showGuiOrInGameMessage(MessageType.ERROR,"boxmatica.error.schematic_projects.failed_to_load_project");
            }
          }
        }
        ...
        GuiTextInput gui=new GuiTextInput(256,"boxmatica.gui.title.create_schematic_project","",GuiUtils.getCurrentScreen(),creator);
        GuiBase.openGui(gui);
    ...
    public enum Type{
      OPEN_MANAGER_GUI("boxmatica.gui.button.schematic_projects.open_manager_gui"),
      LOAD_PROJECT("boxmatica.gui.button.schematic_projects.load_project"),
      CREATE_PROJECT("boxmatica.gui.button.schematic_projects.create_project"),
  ...
  private static class ProjectCreator implements IStringConsumerFeedback{
    ...
    public boolean setString(String projectName) {
      ...
      if(Files.exists(file)==false) {
        ...
        this.gui.addMessage(MessageType.SUCCESS,"boxmatica.message.schematic_projects.project_created",projectName);
        return true;
      }

      InfoUtils.showGuiOrInGameMessage(MessageType.ERROR,"boxmatica.error.schematic_projects.project_already_exists",projectName);
