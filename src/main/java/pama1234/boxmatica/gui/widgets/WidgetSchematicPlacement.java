...
public class WidgetSchematicPlacement extends WidgetListEntryBase<SchematicPlacement>{
  ...
  public void postRenderHovered(int mouseX,int mouseY,boolean selected,DrawContext drawContext) {
    if(this.placement.isLocked()&&
      ...
      String str=StringUtils.translate("boxmatica.hud.schematic_placement.hover_info.placement_locked");
      ...
      GuiBase.isMouseOver(mouseX,mouseY,this.x+this.buttonsStartX-25,this.y+6,11,11)) {
        String str=StringUtils.translate("boxmatica.hud.schematic_placement.hover_info.placement_modified");
        RenderUtils.drawHoverText(mouseX,mouseY,ImmutableList.of(str),drawContext);
        ...
        String fileName=schematicFile!=null?schematicFile.getFileName().toString():StringUtils.translate("boxmatica.gui.label.schematic_placement.in_memory");
        List<String> text=new ArrayList<>();
        text.add(StringUtils.translate("boxmatica.gui.label.schematic_placement.schematic_name",this.placement.getSchematic().getMetadata().getName()));
        text.add(StringUtils.translate("boxmatica.gui.label.schematic_placement.schematic_file",fileName));
        BlockPos o=this.placement.getOrigin();
        String strOrigin=String.format("x: %d, y: %d, z: %d",o.getX(),o.getY(),o.getZ());
        text.add(StringUtils.translate("boxmatica.gui.label.schematic_placement.origin",strOrigin));
        text.add(StringUtils.translate("boxmatica.gui.label.schematic_placement.sub_region_count",String.valueOf(this.placement.getSubRegionCount())));

        Vec3i size=this.placement.getSchematic().getTotalSize();
        String strSize=String.format("%d x %d x %d",size.getX(),size.getY(),size.getZ());
        text.add(StringUtils.translate("boxmatica.gui.label.schematic_placement.enclosing_size",strSize));
  ...
  public static class ButtonListener implements IButtonActionListener{
    ...
    public void actionPerformedWithButton(ButtonBase button,int mouseButton) {
      if(this.type==ButtonType.CONFIGURE) {
        ...
        if(this.widget.placement.isLocked()&&GuiBase.isShiftDown()==false) {
          this.widget.parent.getParentGui().addMessage(MessageType.ERROR,"boxmatica.error.schematic_placements.remove_fail_locked");
        }else {
    ...
    public enum ButtonType{
      CONFIGURE("boxmatica.gui.button.schematic_placements.configure"),
      REMOVE("boxmatica.gui.button.schematic_placements.remove"),
