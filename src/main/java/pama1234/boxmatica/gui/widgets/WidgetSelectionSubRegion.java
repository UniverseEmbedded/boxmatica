...
import pama1234.boxmatica.gui.GuiAreaSelectionEditorSubRegion;
...
public class WidgetSelectionSubRegion extends WidgetListEntryBase<String>{
  ...
  public void postRenderHovered(int mouseX,int mouseY,boolean selected,DrawContext drawContext) {
    ...
    if(this.box!=null) {
      ...
      if(pos1!=null) {
        String str=StringUtils.translate("boxmatica.gui.label.area_editor.pos1");
        text.add(String.format("%s: x: %d, y: %d, z: %d",str,pos1.getX(),pos1.getY(),pos1.getZ()));
      }

      if(pos2!=null) {
        String str=StringUtils.translate("boxmatica.gui.label.area_editor.pos2");
        text.add(String.format("%s: x: %d, y: %d, z: %d",str,pos2.getX(),pos2.getY(),pos2.getZ()));
      }

      if(pos1!=null&&pos2!=null) {
        String str=StringUtils.translate("boxmatica.gui.label.area_editor.dimensions");
  ...
  private static class ButtonListener implements IButtonActionListener{
    ...
    public void actionPerformedWithButton(ButtonBase button,int mouseButton) {
      if(this.type==ButtonType.RENAME) {
        String title="boxmatica.gui.title.rename_area_sub_region";
