...
import pama1234.boxmatica.gui.GuiSchematicBrowserBase;
...
import pama1234.boxmatica.schematic.SchematicSchema;
...
public class WidgetSchematicBrowser extends WidgetFileBrowserBase{
  ...
  public WidgetSchematicBrowser(int x,int y,int width,int height,GuiSchematicBrowserBase parent,@Nullable ISelectionListener<DirectoryEntry> selectionListener) {
    ...
    this.title=StringUtils.translate("boxmatica.gui.title.schematic_browser");
    this.infoWidth=170;
    this.infoHeight=310;
    this.parent=parent;
  }
  ...
  protected void drawSelectedSchematicInfo(@Nullable DirectoryEntry entry,DrawContext drawContext) {
    ...
    if(meta!=null) {
      ...
      String str=StringUtils.translate("boxmatica.gui.label.schematic_info.name");
      ...
      str=StringUtils.translate("boxmatica.gui.label.schematic_info.schematic_author",meta.getAuthor());
      this.drawString(drawContext,str,x,y,textColor);
      y+=12;

      String strDate=DATE_FORMAT.format(new Date(meta.getTimeCreated()));
      str=StringUtils.translate("boxmatica.gui.label.schematic_info.time_created",strDate);
      this.drawString(drawContext,str,x,y,textColor);
      y+=12;

      if(meta.hasBeenModified()) {
        strDate=DATE_FORMAT.format(new Date(meta.getTimeModified()));
        str=StringUtils.translate("boxmatica.gui.label.schematic_info.time_modified",strDate);
        this.drawString(drawContext,str,x,y,textColor);
        y+=12;
      }

      str=StringUtils.translate("boxmatica.gui.label.schematic_info.region_count",meta.getRegionCount());
      this.drawString(drawContext,str,x,y,textColor);
      y+=12;

      if(this.parent.getScreenHeight()>=340) {
        str=StringUtils.translate("boxmatica.gui.label.schematic_info.total_volume",meta.getTotalVolume());
        this.drawString(drawContext,str,x,y,textColor);
        y+=12;

        if(meta.getTotalBlocks()>0) {
          str=StringUtils.translate("boxmatica.gui.label.schematic_info.total_blocks",meta.getTotalBlocks());
          this.drawString(drawContext,str,x,y,textColor);
          y+=12;
        }

        str=StringUtils.translate("boxmatica.gui.label.schematic_info.enclosing_size");
        this.drawString(drawContext,str,x,y,textColor);
        y+=12;
        ...
        if(meta.getTotalBlocks()>0) {
          str=StringUtils.translate("boxmatica.gui.label.schematic_info.total_blocks_and_volume",meta.getTotalBlocks(),meta.getTotalVolume());
          this.drawString(drawContext,str,x,y,textColor);
          y+=12;
        }else {
          str=StringUtils.translate("boxmatica.gui.label.schematic_info.total_volume",meta.getTotalVolume());
        ...
        Vec3i areaSize=meta.getEnclosingSize();
        String tmp=String.format("%d x %d x %d",areaSize.getX(),areaSize.getY(),areaSize.getZ());
        str=StringUtils.translate("boxmatica.gui.label.schematic_info.enclosing_size_value",tmp);
        this.drawString(drawContext,str,x,y,textColor);
        y+=12;
      }
      ...
      if(version!=null) {
        switch(meta.getFileType()) {
          ...
          case VANILLA_STRUCTURE-> {
            str=StringUtils.translate("boxmatica.gui.label.schematic_info.vanilla_version");
            this.drawString(drawContext,str,x,y,textColor);
            y+=12;
          }
          // Not supported
        ...
        if(schema!=null) {
          if(version.minecraftDataVersion()-BoxmaticaSchematic.MINECRAFT_DATA_VERSION>100) {
            str=StringUtils.translate("boxmatica.gui.label.schematic_info.schema.newer",schema.getString(),version.minecraftDataVersion());
          }else {
            str=StringUtils.translate("boxmatica.gui.label.schematic_info.schema",schema.getString(),version.minecraftDataVersion());
          }
          this.drawString(drawContext,str,x,y,textColor);
          y+=12;
        }
      ...
      /*
       * str = StringUtils.translate("boxmatica.gui.label.schematic_info.description");
  ...
  protected SchematicMetadata getSchematicMetadata(DirectoryEntry entry) {
    ...
    if(meta==null&&this.cachedMetadata.containsKey(file)==false) {
      if(entry.getName().endsWith(BoxmaticaSchematic.FILE_EXTENSION)) {
        meta=BoxmaticaSchematic.readMetadataFromFile(entry.getDirectory(),entry.getName());
  ...
  protected Pair<SchematicSchema,SchematicMetadata> getSchematicVersionAndMetadata(DirectoryEntry entry) {
    ...
    if(meta==null&&this.cachedMetadata.containsKey(file)==false) {
      Pair<SchematicSchema,SchematicMetadata> pair=BoxmaticaSchematic.readMetadataAndVersionFromFile(entry.getDirectory(),entry.getName());
    ...
    if(previewImageData!=null&&previewImageData.length>0) {
      ...
      if((size*size)==previewImageData.length) {
        try {
          ...
          Boxmatica.LOGGER.warn("Failed to create a preview image",e);
        }
      }
    }
  }
