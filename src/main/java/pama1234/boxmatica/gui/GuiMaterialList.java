...
import pama1234.boxmatica.gui.widgets.WidgetListMaterialList;
import pama1234.boxmatica.gui.widgets.WidgetMaterialListEntry;
import pama1234.boxmatica.materials.*;
...
public class GuiMaterialList extends GuiListBase<MaterialListEntry,WidgetMaterialListEntry,WidgetListMaterialList>
  ...
  public void initGui() {
    ...
    String str=StringUtils.translate("boxmatica.gui.label.material_list.multiplier");
    ...
    this.addWidget(new WidgetInfoIcon(this.getScreenWidth()-23,10,Icons.INFO_11,"boxmatica.info.material_list"));
    ...
    if(total!=0&&(this.materialList instanceof MaterialListAreaAnalyzer)==false) {
      ...
      String strp;
      String strt=StringUtils.translate("boxmatica.gui.label.material_list.total",total);

      if(missing==0&&mismatch==0) {
        strp=StringUtils.translate("boxmatica.gui.label.material_list.progress.done",String.format("%.0f %%%%",pctDone));
      }else {
        String str1=StringUtils.translate("boxmatica.gui.label.material_list.progress.done",String.format("%.1f %%%%",pctDone));
        String str2=StringUtils.translate("boxmatica.gui.label.material_list.progress.missing",String.format("%.1f %%%%",pctMissing));
        String str3=StringUtils.translate("boxmatica.gui.label.material_list.progress.mismatch",String.format("%.1f %%%%",pctMismatch));
        strp=String.format("%s / %s / %s",str1,str2,str3);
      }

      str=strt+" / "+StringUtils.translate("boxmatica.gui.label.material_list.progress",strp);
  ...
  private int createButton(int x,int y,int width,ButtonListener.Type type) {
    ...
    if(type==ButtonListener.Type.CLEAR_CACHE) {
      button.setHoverStrings("boxmatica.gui.button.hover.material_list.clear_cache");
    }else if(type==ButtonListener.Type.WRITE_TO_FILE) {
      button.setHoverStrings("boxmatica.gui.button.hover.material_list.write_hold_shift_for_csv");
    }
  ...
  private int getElementTotalWidth() {
    ...
    width+=this.getStringWidth(StringUtils.translate("boxmatica.gui.label.material_list.multiplier"));
    width+=130;
  ...
  private static class ButtonListener implements IButtonActionListener{
    ...
    public void actionPerformedWithButton(ButtonBase button,int mouseButton) {
      ...
      switch(this.type) {
        ...
        case CLEAR_CACHE:
          ...
          this.parent.addMessage(MessageType.SUCCESS,3000,"boxmatica.message.material_list.material_cache_cleared");
          break;
        ...
        case WRITE_TO_FILE:
          ...
          if(file!=null) {
            String key="boxmatica.message.material_list_written_to_file";
    ...
    public enum Type{
      REFRESH_LIST("boxmatica.gui.button.material_list.refresh_list"),
      LIST_TYPE("boxmatica.gui.button.material_list.list_type"),
      HIDE_AVAILABLE("boxmatica.gui.button.material_list.hide_available"),
      TOGGLE_INFO_HUD("boxmatica.gui.button.material_list.toggle_info_hud"),
      CLEAR_IGNORED("boxmatica.gui.button.material_list.clear_ignored"),
      CLEAR_CACHE("boxmatica.gui.button.material_list.clear_cache"),
      WRITE_TO_FILE("boxmatica.gui.button.material_list.write_to_file");
