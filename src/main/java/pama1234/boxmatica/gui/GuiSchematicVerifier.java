...
import pama1234.boxmatica.gui.GuiSchematicVerifier.BlockMismatchEntry;
import pama1234.boxmatica.gui.widgets.WidgetListSchematicVerificationResults;
import pama1234.boxmatica.gui.widgets.WidgetSchematicVerificationResult;
import pama1234.boxmatica.render.infohud.InfoHud;
import pama1234.boxmatica.render.infohud.RenderPhase;
...
import pama1234.boxmatica.schematic.verifier.SchematicVerifier.BlockMismatch;
import pama1234.boxmatica.schematic.verifier.SchematicVerifier.MismatchType;
...
public class GuiSchematicVerifier extends GuiListBase<BlockMismatchEntry,WidgetSchematicVerificationResult,WidgetListSchematicVerificationResults>
  ...
  public GuiSchematicVerifier(SchematicPlacement placement) {
    ...
    this.title=StringUtils.translate("boxmatica.gui.title.schematic_verifier",placement.getName());
  ...
  public void initGui() {
    ...
    if(this.verifier.isActive()) {
      String str=StringUtils.translate("boxmatica.gui.label.schematic_verifier.status.verifying",this.verifier.getUnseenChunks(),this.verifier.getTotalChunks());
      ...
      String str=StringUtils.translate("boxmatica.gui.label.schematic_verifier.status.done_errors.no_diff",wb,ws,m,e);

      if(Configs.Generic.ENABLE_DIFFERENT_BLOCKS.getBooleanValue()) {
        str=StringUtils.translate("boxmatica.gui.label.schematic_verifier.status.done_errors",wb,ws,m,e,d);
      }

      this.addLabel(12,y,100,12,0xFFF0F0F0,str);
      str=StringUtils.translate("boxmatica.gui.label.schematic_verifier.status.done_correct_total",c,t);
    ...
    switch(type) {
      case SET_RESULT_MODE_ALL:
        if(Configs.Generic.ENABLE_DIFFERENT_BLOCKS.getBooleanValue()) {
          ...
          label=StringUtils.translate("boxmatica.gui.label.schematic_verifier_display_type.all_not_ignored");
        }
      ...
      case START:
        if(this.verifier.isPaused()) {
          label=StringUtils.translate("boxmatica.gui.button.schematic_verifier.resume");
        }else {
          label=StringUtils.translate("boxmatica.gui.button.schematic_verifier.start");
          enabled=this.verifier.isActive()==false;
        }
        break;

      case STOP:
        label=StringUtils.translate("boxmatica.gui.button.schematic_verifier.stop");
        enabled=this.verifier.isActive();
        break;

      case RESET_VERIFIER: {
        label=StringUtils.translate("boxmatica.gui.button.schematic_verifier.reset_verifier");
      ...
      case SET_LIST_TYPE: {
        ...
        label=StringUtils.translate("boxmatica.gui.button.schematic_verifier.range_type",str);
        break;
      }

      case RESET_IGNORED:
        label=StringUtils.translate("boxmatica.gui.button.schematic_verifier.reset_ignored");
      ...
      case TOGGLE_INFO_HUD: {
        ...
        String str=(val?TXT_GREEN:TXT_RED)+StringUtils.translate("boxmatica.message.value."+(val?"on":"off"))+TXT_RST;
        label=StringUtils.translate("boxmatica.gui.button.schematic_verifier.toggle_info_hud",str);
        break;
      }
  ...
  public void onSelectionChange(@Nullable BlockMismatchEntry entry) {
    if(entry!=null) {
      ...
      if(Configs.InfoOverlays.VERIFIER_OVERLAY_ENABLED.getBooleanValue()==false) {
        ...
        InfoUtils.showGuiOrInGameMessage(MessageType.WARNING,"boxmatica.message.warn.schematic_verifier.overlay_disabled",name,hotkeyName,hotkeyVal);
      }
    }
  }
      ...
      switch(this.type) {
        ...
        case START:
          ...
          if(world!=null) {
            ...
            this.parent.addMessage(MessageType.ERROR,"boxmatica.error.generic.schematic_world_not_loaded");
          }
