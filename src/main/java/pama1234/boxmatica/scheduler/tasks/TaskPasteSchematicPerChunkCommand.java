...
import pama1234.boxmatica.mixin.block.IMixinAbstractBlock;
...
import pama1234.boxmatica.util.PasteNbtBehavior;
...
public class TaskPasteSchematicPerChunkCommand extends TaskPasteSchematicPerChunkBase{
  ...
  protected boolean shouldSetBlock(BlockState stateSchematic,BlockState stateClient) {
    ...
    //        Boxmatica.LOGGER.error("shouldSetBlock(): matched: [{}]/CBOnly [{}], client: [{}], schem: [{}]", matched, this.changedBlockOnly, stateClient.toString(), stateSchematic.toString());
  ...
  protected void onStop() {
    if(this.finished) {
      if(this.printCompletionMessage) {
        if(this.useWorldEdit) {
          InfoUtils.showGuiOrActionBarMessage(MessageType.INFO,"boxmatica.message.schematic_pasted_using_world_edit",this.sentSetblockCommands+this.sentFillCommands);
        }else if(this.useFillCommand) {
          InfoUtils.showGuiOrActionBarMessage(MessageType.INFO,"boxmatica.message.schematic_pasted_using_fill_and_setblock",this.sentFillCommands,this.sentSetblockCommands);
        }else {
          InfoUtils.showGuiOrActionBarMessage(MessageType.INFO,"boxmatica.message.schematic_pasted_using_setblock",this.sentSetblockCommands);
        }
      }
    }else {
      InfoUtils.showGuiOrActionBarMessage(MessageType.ERROR,"boxmatica.message.error.schematic_paste_failed");
    }
  ...
  protected static boolean preparePickedStack(BlockPos pos,BlockState state,BlockEntity be,
    ...
    ItemStack stack=((IMixinAbstractBlock)state.getBlock()).boxmatica_getPickStack(world,pos,state,false);
