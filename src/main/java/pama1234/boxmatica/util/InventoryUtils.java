...
public class InventoryUtils{
  ...
  public static void setPickedItemToHand(int sourceSlot,ItemStack stack,MinecraftClient mc) {
    ...
    if(PlayerInventory.isValidHotbarIndex(sourceSlot)) {
      ...
      if(PICK_BLOCKABLE_SLOTS.size()==0) {
        InfoUtils.showGuiOrInGameMessage(MessageType.WARNING,"boxmatica.message.warn.pickblock.no_valid_slots_configured");
        return;
      }
      ...
      if(hotbarSlot!=-1) {
        ...
        InfoUtils.showGuiOrInGameMessage(MessageType.WARNING,"boxmatica.message.warn.pickblock.no_suitable_slot_found");
      }
    }
  }
  ...
  public static @Nullable InventoryOverlay.Context getTargetInventory(World world,BlockPos pos) {
    ...
    if(blockTmp instanceof BlockEntityProvider) {
      ...
      //            Boxmatica.LOGGER.warn("getTarget():2: pos [{}], be [{}], nbt [{}]", pos.toShortString(), be != null, nbt != null);
  ...
  private static @Nullable InventoryOverlay.Context getTargetInventoryFromBlock(World world,BlockPos pos,@Nullable BlockEntity be,NbtCompound nbt) {
    ...
    //        Boxmatica.LOGGER.warn("getTarget(): [SchematicWorld? {}] pos [{}], inv [{}], be [{}], nbt [{}]", world instanceof WorldSchematic ? "YES" : "NO", pos.toShortString(), inv != null, be != null, nbt != null ? nbt.getString("id") : new NbtCompound());
