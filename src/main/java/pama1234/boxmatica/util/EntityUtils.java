...
import pama1234.boxmatica.mixin.world.IMixinWorld;
...
public class EntityUtils{
  ...
  /**
   * Checks if the requested item is currently in the player's hand such that it would be used for
   * using/placing.
   * This means, that it must either be in the main hand, or the main hand must be empty and the
   * item is in the offhand.
   * 
   * @param player
   * @param stack
   * @return
  ...
  private static Entity createEntityFromNBTSingle(NbtCompound nbt,World world) {
    try {
      ...
      if(optional.isPresent()) {
        ...
        //                Boxmatica.LOGGER.warn("[EntityUtils] createEntityFromNBTSingle() successful; type: [{}]", entity.getType().getName().getString());

        return entity;
      }
    }catch(Exception err) {
      Boxmatica.LOGGER.error("createEntityFromNBTSingle: Exception; {}",err.getLocalizedMessage());
    }
  ...
  public static void loadNbtIntoEntity(Entity entity,NbtCompound nbt) {
    ...
    if(entity instanceof Leashable) {
      ...
      ((IMixinEntity)entity).boxmatica_readCustomDataFromNbt(nbt);
    }
  }
  ...
  private static void readLeashableEntityCustomData(Entity entity,NbtCompound nbt) {
    ...
    if(leashable.getLeashData()!=null&&leashable.getLeashData().unresolvedLeashData!=null) {
      leashable.getLeashData().unresolvedLeashData
        ...
        leashable.attachLeash(((IMixinWorld)mc.world).boxmatica_getEntityLookup().get(uuid),false))
        .ifRight(pos->leashable.attachLeash(LeashKnotEntity.getOrCreate(mc.world,pos),false));
    }
  }
  ...
  /**
   * Checks if the requested item is currently in the entity's hand such that it would be used for
   * using/placing.
   * This means, that it must either be in the main hand, or the main hand must be empty and the
   * item is in the offhand.
   * 
