...
import pama1234.boxmatica.scheduler.tasks.TaskDeleteArea;
import pama1234.boxmatica.scheduler.tasks.TaskDeleteBlocksByPlacement;
import pama1234.boxmatica.scheduler.tasks.TaskFillArea;
  ...
  public static void fillSelectionVolumes(MinecraftClient mc,BlockState state,@Nullable BlockState stateToReplace) {
    if(mc.player!=null&&EntityUtils.isCreativeMode(mc.player)) {
      ...
      if(area.getAllSubRegionBoxes().size()>0) {
        ...
        InfoUtils.showGuiOrInGameMessage(MessageType.ERROR,"boxmatica.message.error.empty_area_selection");
      }
    }else {
