...
import pama1234.boxmatica.mixin.input.IMixinKeyBinding;
...
public class EasyPlaceUtils{
  ...
  private static boolean hasUseAction(Block block) {
    ...
    if(val==null) {
      try {
        ...
        Boxmatica.LOGGER.warn("EasyPlaceUtils: Failed to reflect method Block::onUse",e);
        val=false;
      }
  ...
  public static void easyPlaceOnUseTick() {
    InputUtil.Key useKey=((IMixinKeyBinding)MinecraftClient.getInstance().options.useKey).boxmatica_getBoundKey();
  ...
  public static boolean handleEasyPlaceWithMessage() {
    ...
    if(isFirstClickEasyPlace&&result==ActionResult.FAIL) {
      ...
      //MessageDispatcher.warning(1500).type(output).translate("boxmatica.message.easy_place_fail");
  ...
  public static boolean handlePlacementRestriction() {
    ...
    if(cancel&&isFirstClickPlacementRestriction) {
      //MessageOutput output = Configs.InfoOverlays.EASY_PLACE_WARNINGS.getValue();
      //MessageDispatcher.warning(1000).type(output).translate("boxmatica.message.placement_restriction_fail");
