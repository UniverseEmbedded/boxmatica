...
import pama1234.boxmatica.materials.MaterialCache;
import pama1234.boxmatica.mixin.entity.IMixinSignBlockEntity;
import pama1234.boxmatica.mixin.block.IMixinWallMountedBlock;
import pama1234.boxmatica.schematic.BoxmaticaSchematic;
import pama1234.boxmatica.schematic.SchematicMetadata;
import pama1234.boxmatica.schematic.SchematicaSchematic;
...
import pama1234.boxmatica.schematic.placement.SchematicPlacementManager.PlacementPart;
...
import pama1234.boxmatica.util.PositionUtils.Corner;
import pama1234.boxmatica.util.RayTraceUtils.RayTraceWrapper;
import pama1234.boxmatica.util.RayTraceUtils.RayTraceWrapper.HitType;
...
public class WorldUtils{
  ...
  public static boolean shouldPreventBlockUpdates(World world) {
    return ((IWorldUpdateSuppressor)world).boxmatica_getShouldPreventBlockUpdates();
  }

  public static void setShouldPreventBlockUpdates(World world,boolean preventUpdates) {
    ((IWorldUpdateSuppressor)world).boxmatica_setShouldPreventBlockUpdates(preventUpdates);
  }

  public static boolean convertBoxmaticaSchematicToBoxmaticaSchematic(
    ...
    BoxmaticaSchematic boxmaticaSchematic=convertBoxmaticaSchematicToBoxmaticaSchematic(inputDir,inputFileName,outputFileName,feedback);
    return boxmaticaSchematic!=null&&boxmaticaSchematic.writeToFile(outputDir,outputFileName,override);
  }

  public static boolean convertSpongeSchematicToBoxmaticaSchematic(
    Path inputDir,String inputFileName,Path outputDir,String outputFileName,boolean ignoreEntities,boolean override,IStringConsumer feedback) {
    ...
    BoxmaticaSchematic origSchematic=convertSpongeSchematicToBoxmaticaSchematic(inputDir,inputFileName);

    if(origSchematic==null) {
      feedback.setString("boxmatica.error.schematic_conversion.sponge_to_boxmatica.failed_to_read_sponge");
    ...
    BoxmaticaSchematic.SchematicSaveInfo info=new BoxmaticaSchematic.SchematicSaveInfo(false,false);
  ...
  public static boolean convertSchematicaSchematicToBoxmaticaSchematic(
    Path inputDir,String inputFileName,Path outputDir,String outputFileName,boolean ignoreEntities,boolean override,IStringConsumer feedback) {
    BoxmaticaSchematic boxmaticaSchematic=convertSchematicaSchematicToBoxmaticaSchematic(inputDir,inputFileName,ignoreEntities,feedback);
    return boxmaticaSchematic!=null&&boxmaticaSchematic.writeToFile(outputDir,outputFileName,override);
  }

  @Nullable
  public static BoxmaticaSchematic convertBoxmaticaSchematicToBoxmaticaSchematic(Path inputDir,String inputFileName,
    ...
    newSchematic.getMetadata().setDescription("Converted Boxmatic V"+origMetadata.getSchematicVersion()+", Schema "+origMetadata.getSchemaString());
  ...
  @Nullable
  public static BoxmaticaSchematic convertSchematicaSchematicToBoxmaticaSchematic(Path inputDir,String inputFileName,
    ...
    if(schematic==null) {
      feedback.setString("boxmatica.error.schematic_conversion.schematic_to_boxmatica.failed_to_read_schematic");
      Configs.Generic.DATAFIXER_MODE.setOptionListValue(oldMode);
      return null;
    }
    ...
    BoxmaticaSchematic newSchematic=BoxmaticaSchematic.createFromWorld(world,area,info,"?",feedback);

    if(newSchematic!=null&&ignoreEntities==false) {
      newSchematic.takeEntityDataFromSchematicaSchematic(schematic,subRegionName);
    }else {
      feedback.setString("boxmatica.error.schematic_conversion.schematic_to_boxmatica.failed_to_create_schematic");
    }
  ...
  public static boolean convertStructureToBoxmaticaSchematic(Path structureDir,String structureFileName,
    Path outputDir,String outputFileName,boolean override) {
    BoxmaticaSchematic boxmaticaSchematic=convertStructureToBoxmaticaSchematic(structureDir,structureFileName);
    return boxmaticaSchematic!=null&&boxmaticaSchematic.writeToFile(outputDir,outputFileName,override);
  }
  ...
  public static boolean convertStructureToBoxmaticaSchematic(Path structureDir,String structureFileName,
    ...
    DataFixerMode oldMode=(DataFixerMode)Configs.Generic.DATAFIXER_MODE.getOptionListValue();
    Configs.Generic.DATAFIXER_MODE.setOptionListValue(DataFixerMode.ALWAYS);
    BoxmaticaSchematic origStructure=convertStructureToBoxmaticaSchematic(structureDir,structureFileName);

    if(origStructure==null) {
      feedback.setString("boxmatica.error.schematic_conversion.structure_to_boxmatica.failed_to_read_structure");
      Configs.Generic.DATAFIXER_MODE.setOptionListValue(oldMode);
      return false;
    }
  ...
  @Nullable
  public static BoxmaticaSchematic convertSpongeSchematicToBoxmaticaSchematic(Path dir,String fileName) {
    try {
      BoxmaticaSchematic schematic=BoxmaticaSchematic.createFromFile(dir,fileName,FileType.SPONGE_SCHEMATIC);
  ...
  @Nullable
  public static BoxmaticaSchematic convertStructureToBoxmaticaSchematic(Path structureDir,String structureFileName) {
    try {
      BoxmaticaSchematic boxmaticaSchematic=BoxmaticaSchematic.createFromFile(structureDir,structureFileName,FileType.VANILLA_STRUCTURE);

      if(boxmaticaSchematic==null) {
        InfoUtils.showGuiOrInGameMessage(MessageType.ERROR,"Failed to read the vanilla structure template from '"+structureFileName+'"');
      }

      return boxmaticaSchematic;
    }catch(Exception e) {
      InfoUtils.showGuiOrInGameMessage(MessageType.ERROR,"Exception while trying to load the vanilla structure: "+e.getMessage());
      Boxmatica.LOGGER.error("Exception while trying to load the vanilla structure: "+e.getMessage());
    }

    return null;
  }

  public static boolean convertBoxmaticaSchematicToSchematicaSchematic(
    Path inputDir,String inputFileName,Path outputDir,String outputFileName,boolean ignoreEntities,boolean override,IStringConsumer feedback) {
    //SchematicaSchematic schematic = convertBoxmaticaSchematicToSchematicaSchematic(inputDir, inputFileName, ignoreEntities, feedback);
  ...
  public static boolean convertBoxmaticaSchematicToV6BoxmaticaSchematic(
    ...
    BoxmaticaSchematic v6BoxmaticaSchematic=BoxmaticaSchematic.createEmptySchematicFromExisting(v7BoxmaticaSchematic,MinecraftClient.getInstance().player.getName().getString());
    v6BoxmaticaSchematic.downgradeV7toV6Schematic(v7BoxmaticaSchematic);
  ...
  public static boolean convertBoxmaticaSchematicToVanillaStructure(
    Path inputDir,String inputFileName,Path outputDir,String outputFileName,boolean ignoreEntities,boolean override,IStringConsumer feedback) {
    StructureTemplate template=convertBoxmaticaSchematicToVanillaStructure(inputDir,inputFileName,ignoreEntities,feedback);
    return writeVanillaStructureToFile(template,outputDir,outputFileName,override,feedback);
  }

  @Nullable
  public static StructureTemplate convertBoxmaticaSchematicToVanillaStructure(Path inputDir,String inputFileName,boolean ignoreEntities,IStringConsumer feedback) {
    BoxmaticaSchematic boxmaticaSchematic=BoxmaticaSchematic.createFromFile(inputDir,inputFileName);
    ...
    BlockPos size=new BlockPos(boxmaticaSchematic.getTotalSize());
    loadChunksSchematicWorld(world,BlockPos.ORIGIN,size);
    SchematicPlacement schematicPlacement=SchematicPlacement.createForSchematicConversion(boxmaticaSchematic,BlockPos.ORIGIN);
    boxmaticaSchematic.placeToWorld(world,schematicPlacement,false); // TODO use a per-chunk version for a bit more speed
  ...
  private static boolean writeVanillaStructureToFile(StructureTemplate template,Path dir,String fileNameIn,boolean override,IStringConsumer feedback) {
    ...
    try {
      ...
      if(!Files.isDirectory(dir)) {
        feedback.setString(StringUtils.translate("boxmatica.error.schematic_write_to_file_failed.directory_creation_failed",dir.toAbsolutePath()));
        return false;
      }

      if(override==false&&Files.exists(file)) {
        feedback.setString(StringUtils.translate("boxmatica.error.structure_write_to_file_failed.exists",file.toAbsolutePath()));
        return false;
      }
      ...
      feedback.setString(StringUtils.translate("boxmatica.error.structure_write_to_file_failed.exception",file.toAbsolutePath()));
    }
  ...
  /**
   * Does a ray trace to the schematic world, and returns either the closest or the furthest hit
   * block.
   * 
   * @param closest
   * @param mc
  ...
  public static void insertSignTextFromSchematic(SignBlockEntity beClient,String[] screenTextArr,boolean front) {
    ...
    if(worldSchematic!=null) {
      ...
      if(beSchem instanceof SignBlockEntity) {
        ...
        SignText textSchematic=front?beMixinSchem.boxmatica_getFrontText():beMixinSchem.boxmatica_getBackText();
    ...
    //Configs.Generic.EASY_PLACE_POST_REWRITE.getBooleanValue() == false &&
      ...
      if(result==ActionResult.FAIL) {
        ...
        if(type==MessageOutputType.MESSAGE) {
          InfoUtils.showGuiOrInGameMessage(MessageType.WARNING,"boxmatica.message.easy_place_fail");
        }else if(type==MessageOutputType.ACTIONBAR) {
          InfoUtils.printActionbarMessage("boxmatica.message.easy_place_fail");
        }
  ...
  public static PlacementProtocolData applyPlacementProtocolAll(BlockPos pos,BlockState stateSchematic,Vec3d hitVecIn) {
    ...
    if(stateBlock instanceof AbstractTorchBlock||
      ...
      //If the supporting block doesn't exist, fail
      if(!((IMixinWallMountedBlock)stateBlock).boxmatica_invokeCanPlaceAt(stateSchematic,world,pos)) placementData.mustFail=true;
    }
  ...
  public static <T extends Comparable<T>> Vec3d applyPlacementProtocolV3(BlockPos pos,BlockState state,Vec3d hitVecIn) {
    ...
    try {
      ...
      Boxmatica.LOGGER.warn("Exception trying to request placement protocol value",e);
    }
  ...
  public static boolean handlePlacementRestriction(MinecraftClient mc) {
    ...
    if(cancel) {
      ...
      if(type==MessageOutputType.MESSAGE) {
        InfoUtils.showGuiOrInGameMessage(MessageType.WARNING,"boxmatica.message.placement_restriction_fail");
      }else if(type==MessageOutputType.ACTIONBAR) {
        InfoUtils.printActionbarMessage("boxmatica.message.placement_restriction_fail");
      }
    }
  ...
  private static boolean placementRestrictionInEffect(MinecraftClient mc) {
    ...
    if(trace!=null&&trace.getType()==HitResult.Type.BLOCK) {
      ...
      if(schematicBlock instanceof WallTorchBlock||
        ...
        if(blockHitResult.getSide()!=stateSchematic.get(Properties.HORIZONTAL_FACING)) return true;
      }
  ...
  private static boolean isMatchingStatePlacementRestriction(BlockState state1,BlockState state2) {
    ...
    for(Property<?> property:orientationProperties) {
      ...
      if(!hasProperty1) continue;

      if(state1.get(property)!=state2.get(property)) return false;
    }
