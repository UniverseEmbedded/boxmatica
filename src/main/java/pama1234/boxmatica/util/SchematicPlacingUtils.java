...
public class SchematicPlacingUtils{
  public static boolean placeToWorldWithinChunk(World world,
    ...
    BoxmaticaSchematic schematic=schematicPlacement.getSchematic();
    ...
    try {
      ...
      for(String regionName:regionsTouchingChunk) {
        BoxmaticaBlockStateContainer container=schematic.getSubRegionContainer(regionName);
        ...
        if(placement.isEnabled()) {
          ...
          if(placeBlocksWithinChunk(world,chunkPos,regionName,container,blockEntityMap,
            ...
            allSuccess=false;
            Boxmatica.LOGGER.warn("Invalid/missing schematic data in schematic '{}' for sub-region '{}'",schematic.getMetadata().getName(),regionName);
          }
  ...
  public static boolean placeBlocksWithinChunk(World world,ChunkPos chunkPos,String regionName,
    BoxmaticaBlockStateContainer container,
      ...
      for(int z=startZ;z<=endZ;++z) {
        for(int x=startX;x<=endX;++x) {
          ...
          if(!shouldPasteBlock(pos,layerBehavior)) {
            //                        Boxmatica.LOGGER.error("placeBlocksWithinChunk(): Skipping block at pos [{}]", pos.toShortString());
            continue;
          }
          ...
          if(world.setBlockState(pos,state,0x12)&&teNBT!=null) {
            ...
            if(te!=null) {
              ...
              try {
                ...
                Boxmatica.LOGGER.warn("Failed to load BlockEntity data for {} @ {}",state,pos);
              }
            }
          }
        }
  ...
  public static void placeEntitiesToWorldWithinChunk(World world,ChunkPos chunkPos,
    ...
    for(EntityInfo info:entityList) {
      ...
      if(!shouldPasteEntity(new Vec3d(x,y,z),layerBehavior)) {
        //                Boxmatica.LOGGER.error("placeEntitiesToWorldWithinChunk(): Skipping Entity at pos [{}]", pos.toString());
        continue;
      }
