...
import pama1234.boxmatica.mixin.screen.IMixinHandledScreen;
...
public class MaterialListHudRenderer implements IInfoHudRenderer{
  ...
  public int render(int xOffset,int yOffset,HudAlignment alignment,DrawContext drawContext) {
    ...
    String title=GuiBase.TXT_BOLD+StringUtils.translate("boxmatica.gui.button.material_list")+GuiBase.TXT_RST;
  ...
  protected String getFormattedCountString(int count,int maxStackSize) {
    ...
    if(count>maxStackSize) {
      if(boxCount>=1.0) {
        return String.format("%d (%.2f %s)",count,boxCount,StringUtils.translate("boxmatica.gui.label.material_list.abbr.shulker_box"));
      }else if(remainder>0) {
  ...
  public static void highlightSlotsWithItem(ItemStack referenceItem,HandledScreen<?> gui,Color4f color,MinecraftClient mc) {
    ...
    //        RenderUtils.blend(true);
    int guiX=((IMixinHandledScreen)gui).boxmatica_getX();
    int guiY=((IMixinHandledScreen)gui).boxmatica_getY();
