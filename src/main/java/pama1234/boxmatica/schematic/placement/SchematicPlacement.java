...
import pama1234.boxmatica.materials.MaterialListPlacement;
...
public class SchematicPlacement{
  ...
  public SchematicPlacement(BoxmaticaSchematic schematic,BlockPos origin,String name,boolean enabled,boolean enableRender) {
    this(schematic,origin,name,enabled,enableRender,DataManager.getSchematicPlacementManager());
  }

  public SchematicPlacement(BoxmaticaSchematic schematic,BlockPos origin,String name,boolean enabled,boolean enableRender,SchematicPlacementManager placementManager) {
  ...
  public static SchematicPlacement createFor(BoxmaticaSchematic schematic,BlockPos origin,String name,boolean enabled,boolean enableRender) {
   ...
   * @return
   */
  public static SchematicPlacement createForSchematicConversion(BoxmaticaSchematic schematic,BlockPos origin) {
   ...
   * @return
   */
  public static SchematicPlacement createTemporary(BoxmaticaSchematic schematic,BlockPos origin) {
  ...
  public BoxmaticaSchematic getSchematic() {
    return schematic;
  }
  ...
  public ImmutableMap<String,Box> getSubRegionBoxes(RequiredEnabled required) {
    ...
    for(Map.Entry<String,SubRegionPlacement> entry:this.relativeSubRegionPlacements.entrySet()) {
      ...
      if(areaSize==null) {
        Boxmatica.LOGGER.warn("SchematicPlacement.getSubRegionBoxes(): Size for sub-region '{}' not found in the schematic '{}'",name,this.schematic.getMetadata().getName());
        continue;
      }
    ...
    if(placement!=null) {
      if(placement.matchesRequirement(required)) {
        ...
        if(areaSize!=null) {
          ...
          Boxmatica.LOGGER.warn("SchematicPlacement.getSubRegionBoxFor(): Size for sub-region '{}' not found in the schematic '{}'",regionName,this.schematic.getMetadata().getName());
        }
      }
    }
  ...
  public void moveSubRegionTo(String regionName,BlockPos newPos,IStringConsumer feedback) {
    if(this.isLocked()) {
      feedback.setString("boxmatica.message.placement.cant_modify_is_locked");
      return;
    }
  ...
  public void setSubRegionRotation(String regionName,BlockRotation rotation,IMessageConsumer feedback) {
    if(this.isLocked()) {
      feedback.addMessage(MessageType.ERROR,"boxmatica.message.placement.cant_modify_is_locked");
      return;
    }
  ...
  public static SchematicPlacement fromJson(JsonObject obj) {
    if(JsonUtils.hasString(obj,"schematic")&&
      ...
      BoxmaticaSchematic schematic=SchematicHolder.getInstance().getOrLoad(file);

      if(schematic==null) {
        Boxmatica.LOGGER.warn("Failed to load schematic '{}'",file.toAbsolutePath());
        return null;
      }
      ...
      if(posArr.size()!=3) {
        Boxmatica.LOGGER.warn("Failed to load schematic placement for '{}', invalid origin position",file.toAbsolutePath());
        return null;
      }
  ...
  public static @Nullable SchematicPlacement createFromNbt(@Nonnull BoxmaticaSchematic schematic,NbtCompound nbt) {
    String name=nbt.getString("Name","?");
    BlockPos origin=NbtUtils.readBlockPosFromArrayTag(nbt,"Origin");
    BlockRotation rot=BlockRotation.values()[nbt.getInt("Rotation",0)];
    BlockMirror mirror=BlockMirror.values()[nbt.getInt("Mirror",0)];
