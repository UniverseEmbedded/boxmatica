package pama1234.boxmatica.schematic.placement;
...
import pama1234.boxmatica.scheduler.tasks.TaskPasteSchematicPerChunkBase;
import pama1234.boxmatica.scheduler.tasks.TaskPasteSchematicPerChunkCommand;
import pama1234.boxmatica.scheduler.tasks.TaskPasteSchematicPerChunkDirect;
import pama1234.boxmatica.scheduler.tasks.TaskPasteSchematicSetblockToMcfunction;
...
import pama1234.boxmatica.util.PositionUtils.ChunkPosDistanceComparator;
...
public class SchematicPlacementManager{
  ...
  public void processQueuedChunks() {
    ...
    if(this.hasQueuedChunks()) {
      ...
      BoxmaticaRenderer.getInstance().getWorldRenderer().markNeedsUpdate();
    }
  }
  ...
  public void addSchematicPlacement(SchematicPlacement placement,boolean printMessages) {
    if(this.schematicPlacements.contains(placement)==false) {
      ...
      if(printMessages) {
        InfoUtils.showGuiMessage(MessageType.SUCCESS,StringUtils.translate("boxmatica.message.schematic_placement_created",placement.getName()));
        ...
        if(Configs.InfoOverlays.WARN_DISABLED_RENDERING.getBooleanValue()) {
          ...
          if(mode!=LayerMode.ALL) {
            InfoUtils.showGuiAndInGameMessage(MessageType.WARNING,"boxmatica.message.warn.layer_mode_currently_at",mode.getDisplayName());
          }
          ...
          if(Configs.Visuals.ENABLE_RENDERING.getBooleanValue()==false) {
            ...
            InfoUtils.showGuiAndInGameMessage(MessageType.WARNING,8000,
              "boxmatica.message.warn.main_rendering_disabled",configName,hotkeyName,hotkeyVal);
          }
          ...
          if(Configs.Visuals.ENABLE_SCHEMATIC_RENDERING.getBooleanValue()==false) {
            ...
            InfoUtils.showGuiAndInGameMessage(MessageType.WARNING,8000,
              "boxmatica.message.warn.schematic_rendering_disabled",configName,hotkeyName,hotkeyVal);
          }
          ...
          if(Configs.Visuals.ENABLE_SCHEMATIC_BLOCKS.getBooleanValue()==false) {
            ...
            InfoUtils.showGuiAndInGameMessage(MessageType.WARNING,8000,
              "boxmatica.message.warn.schematic_blocks_rendering_disabled",configName,hotkeyName,hotkeyVal);
          }
        }
      }
    }else if(printMessages) {
      InfoUtils.showGuiAndInGameMessage(MessageType.ERROR,"boxmatica.error.duplicate_schematic_placement");
    }
  }
  ...
  public List<SchematicPlacement> getAllPlacementsOfSchematic(BoxmaticaSchematic schematic) {
  ...
  public void removeAllPlacementsOfSchematic(BoxmaticaSchematic schematic) {
  ...
  public void markAllPlacementsOfSchematicForRebuild(BoxmaticaSchematic schematic) {
    for(SchematicPlacement placement:this.schematicPlacements) {
      if(placement.getSchematic()==schematic) {
        this.markChunksForRebuild(placement);
      }
  ...
  public void setPositionOfCurrentSelectionTo(BlockPos pos,MinecraftClient mc) {
    ...
    if(schematicPlacement!=null) {
      if(schematicPlacement.isLocked()) {
        InfoUtils.showGuiOrActionBarMessage(MessageType.ERROR,"boxmatica.message.placement.cant_modify_is_locked");
        return;
      }
      ...
      if(movingBox) {
        ...
        InfoUtils.showGuiOrActionBarMessage(MessageType.SUCCESS,"boxmatica.message.placement.moved_subregion_to",posStr);
      }
      // Moving the origin point
        ...
        if(old.equals(schematicPlacement.getOrigin())==false) {
          ...
          InfoUtils.showGuiOrActionBarMessage(MessageType.SUCCESS,"boxmatica.message.placement.moved_placement_origin",posStrOld,posStrNew);
        }
      }
    }
  }
  ...
  public void pastePlacementToWorld(final SchematicPlacement schematicPlacement,boolean changedBlocksOnly,boolean printMessage,MinecraftClient mc) {
    if(mc.player!=null&&EntityUtils.isCreativeMode(mc.player)) {
      if(schematicPlacement!=null) {
         ...
         * "boxmatica.message.error.placement_paste_outside_world");
         * return;
         * }
         */
        LayerRange range=DataManager.getRenderLayerRange();
        ...
        if(Configs.Generic.PASTE_TO_MCFUNCTION.getBooleanValue()) {
          ...
          if(EntitiesDataStorage.getInstance().hasServuxServer()&&
            ...
            Boxmatica.debugLog("Found a Servux server, I am sending the Schematic Placement to it.");
            InfoUtils.showGuiOrActionBarMessage(MessageType.INFO,"boxmatica.message.paste_with_servux");
            ...
            if(nbt.getSizeInBytes()>maxSize) {
              Boxmatica.LOGGER.warn("[Servux Paste]: Slicing Oversided Schematic for Servux Paste ...");
              this.sliceForServux(schematicPlacement.getSchematic(),nbt,maxSize,printMessage);
            }else {
              nbt.putString("Task","BoxmaticaPaste");
              ServuxBoxmaticaHandler.getInstance().encodeClientData(ServuxBoxmaticaPacket.ResponseC2SStart(nbt));
            }
          }else {
            ...
            if(printMessage) {
              InfoUtils.showGuiOrActionBarMessage(MessageType.INFO,"boxmatica.message.scheduled_task_added");
            }
          }
        ...
        InfoUtils.showGuiOrInGameMessage(MessageType.ERROR,"boxmatica.message.error.no_placement_selected");
      }
    }else {
      InfoUtils.showGuiOrInGameMessage(MessageType.ERROR,"boxmatica.error.generic.creative_mode_only");
    }
  }
