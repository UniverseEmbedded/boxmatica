...
import pama1234.boxmatica.schematic.transmit.SchematicBuffer;
import pama1234.boxmatica.schematic.transmit.SchematicBufferManager;
import pama1234.boxmatica.mixin.world.IMixinWorldTickScheduler;
import pama1234.boxmatica.network.ServuxBoxmaticaHandler;
import pama1234.boxmatica.network.ServuxBoxmaticaPacket;
import pama1234.boxmatica.schematic.container.IBoxmaticaBlockStatePalette;
import pama1234.boxmatica.schematic.container.BoxmaticaBlockStateContainer;
import pama1234.boxmatica.schematic.conversion.SchematicConversionFixers;
import pama1234.boxmatica.schematic.conversion.SchematicConversionMaps;
import pama1234.boxmatica.schematic.conversion.SchematicConverter;
import pama1234.boxmatica.schematic.conversion.SchematicDowngradeConverter;
import pama1234.boxmatica.schematic.placement.SchematicPlacement;
import pama1234.boxmatica.schematic.placement.SubRegionPlacement;
...
import pama1234.boxmatica.util.*;
...
public class BoxmaticaSchematic{
  ...
  private final Map<String,BoxmaticaBlockStateContainer> blockContainers=new HashMap<>();
  ...
  public BoxmaticaSchematic(Path file,NbtCompound nbt,FileType type) {
    this.readFromNBT(nbt);
    this.schematicFile=file;
    this.schematicType=type;
    this.converter=SchematicConverter.createForBoxmatica();
  }
  ...
  @Nullable
  public static BoxmaticaSchematic createFromWorld(World world,AreaSelection area,SchematicSaveInfo info,
    String author,IStringConsumer feedback) {
    List<Box> boxes=PositionUtils.getValidBoxes(area);

    if(boxes.isEmpty()) {
      feedback.setString(StringUtils.translate("boxmatica.error.schematic.create.no_selections"));
      return null;
    }

    BoxmaticaSchematic schematic=new BoxmaticaSchematic(null);
     ...
     * BoxmaticaBlockStateContainer.CODEC.encodeStart(NbtOps.INSTANCE,
     * schematic.blockContainers.get(entry)).resultOrPartial().orElse(null));
     * }
     * System.out.printf("MetaData --> [%s]\n", SchematicMetadata.CODEC.encodeStart(NbtOps.INSTANCE,
     * schematic.metadata).resultOrPartial().orElse(null));
     */
   ...
   * @return
   */
  public static BoxmaticaSchematic createEmptySchematic(AreaSelection area,String author) {
    List<Box> boxes=PositionUtils.getValidBoxes(area);

    if(boxes.isEmpty()) {
      InfoUtils.showGuiOrInGameMessage(MessageType.ERROR,StringUtils.translate("boxmatica.error.schematic.create.no_selections"));
      return null;
    }
    ...
    for(Box box:boxes) {
      ...
      BoxmaticaBlockStateContainer container=new BoxmaticaBlockStateContainer(sizeX,sizeY,sizeZ);
  ...
  /**
   * Copy an existing Boxmatic to a new object to make a "copy" for file export.
   *
   * @param existing  (Existing Boxmatic object)
   * @param newAuthor
   * @return
   */
  public static BoxmaticaSchematic createEmptySchematicFromExisting(@Nonnull BoxmaticaSchematic existing,String newAuthor) {
    BoxmaticaSchematic newSchematic=new BoxmaticaSchematic(null,existing.schematicType);
  ...
  public boolean downgradeV7toV6Schematic(BoxmaticaSchematic v7Schematic) {
  ...
  public boolean placeToWorld(World world,SchematicPlacement schematicPlacement,boolean notifyNeighbors,boolean ignoreEntities) {
    ...
    for(String regionName:relativePlacements.keySet()) {
      ...
      if(placement.isEnabled()) {
        ...
        BoxmaticaBlockStateContainer container=this.blockContainers.get(regionName);
        ...
        if(regionPos!=null&&regionSize!=null&&container!=null&&tileMap!=null) {
          ...
          Boxmatica.LOGGER.warn("Invalid/missing schematic data in schematic '{}' for sub-region '{}'",this.metadata.getName(),regionName);
        }
        ...
        if(ignoreEntities==false&&schematicPlacement.ignoreEntities()==false&&
          ...
          Boxmatica.LOGGER.error("[Schem] Unable to place entites to world. (Ignore entities on?)");
        }
      }
    }
  ...
  private boolean placeBlocksToWorld(World world,BlockPos origin,BlockPos regionPos,BlockPos regionSize,
    ...
    BoxmaticaBlockStateContainer container,Map<BlockPos,NbtCompound> tileMap,
          ...
          if(world.setBlockState(pos,state,0x12)&&teNBT!=null) {
            ...
            if(te!=null) {
              ...
              try {
                ...
                Boxmatica.LOGGER.warn("Failed to load TileEntity data for {} @ {}",state,pos);
              }
            }
          }
        }
     ...
     * posMinRel.getY() + y - regionPos.getY(),
     * posMinRel.getZ() + z - regionPos.getZ());
     * BlockPos pos = PositionUtils.getTransformedPlacementPosition(posMutable, schematicPlacement,
     * placement).add(origin);
     ...
     * serverWorld.getBlockTickScheduler().scheduleTick(new OrderedTick<>(tick.type(), pos, (int)
     ...
     * serverWorld.getFluidTickScheduler().scheduleTick(new OrderedTick<>(tick.type(), pos, (int)
     * tick.triggerTick(), tick.priority(), tick.subTickOrder()));
     * }
     * }
     * }
  ...
  private void placeEntitiesToWorld(World world,BlockPos origin,BlockPos regionPos,BlockPos regionSize,SchematicPlacement schematicPlacement,SubRegionPlacement placement,List<EntityInfo> entityList) {
    ...
    Boxmatica.LOGGER.warn("[Schem] placeEntitiesToWorld: entityList size [{}]",entityList.size());
    ...
    for(EntityInfo info:entityList) {
      ...
      if(entity!=null) {
        ...
        Boxmatica.LOGGER.warn("[Schem] placeEntitiesToWorld: entity [{}]",entity.getType().getName().getString());

        SchematicPlacingUtils.rotateEntity(entity,x,y,z,rotationCombined,mirrorMain,mirrorSub);
        EntityUtils.spawnEntityAndPassengersInWorld(entity,world);
      }else {
        Boxmatica.LOGGER.error("[Schem] placeEntitiesToWorld: entity == null!");
      }
    }
  }
  ...
  private void takeBlocksFromWorld(World world,List<Box> boxes,SchematicSaveInfo info) {
    ...
    for(Box box:boxes) {
      ...
      if(world instanceof ServerWorld serverWorld) {
        ...
        this.getTicksFromScheduler(((IMixinWorldTickScheduler<Block>)serverWorld.getBlockTickScheduler()).boxmatica_getChunkTickSchedulers(),
          blockTickMap,tickBox,minCorner,currentTick);

        this.getTicksFromScheduler(((IMixinWorldTickScheduler<Fluid>)serverWorld.getFluidTickScheduler()).boxmatica_getChunkTickSchedulers(),
  ...
  public void takeBlocksFromWorldWithinChunk(World world,ImmutableMap<String,IntBoundingBox> volumes,
    ...
    for(Map.Entry<String,IntBoundingBox> volumeEntry:volumes.entrySet()) {
      ...
      if(box==null) {
        Boxmatica.LOGGER.error("null Box for sub-region '{}' while trying to save chunk-wise schematic",regionName);
        continue;
      }
      ...
      if(container==null||tileEntityMap==null||blockTickMap==null||fluidTickMap==null) {
        Boxmatica.LOGGER.error("null map(s) for sub-region '{}' while trying to save chunk-wise schematic",regionName);
        continue;
      }
  ...
  @Nullable
  public BoxmaticaBlockStateContainer getSubRegionContainer(String regionName) {
  ...
  private NbtCompound writeSubRegionsToNBT() {
    ...
    if(this.blockContainers.isEmpty()==false) {
      for(String regionName:this.blockContainers.keySet()) {
        BoxmaticaBlockStateContainer blockContainer=this.blockContainers.get(regionName);
        Map<BlockPos,NbtCompound> tileMap=this.tileEntities.get(regionName);
        List<EntityInfo> entityList=this.entities.get(regionName);
  ...
  public void sendTransmitFile(NbtCompound nbtIn,final long sessionKey,boolean printMessage) {
    if(EntitiesDataStorage.getInstance().hasServuxServer()==false) {
      if(printMessage) {
        InfoUtils.showGuiOrInGameMessage(MessageType.ERROR,"boxmatica.message.error.schematic_transmit_not_available");
      }

      Boxmatica.LOGGER.error("transmitFileToServux: Cannot transmit a Boxmatic without having Servux present.");
      return;
    }

    if(printMessage) {
      InfoUtils.showGuiOrInGameMessage(MessageType.INFO,"boxmatica.message.schematic_transmit_start");
    }

    Path file=this.getFile();
    NbtCompound output=new NbtCompound();

    output.putString("Task","Boxmatic-TransmitStart");
    ...
    ServuxBoxmaticaHandler.getInstance().encodeClientData(ServuxBoxmaticaPacket.ResponseC2SStart(output));
    ...
    try(InputStream is=Files.newInputStream(file)) {
      int bytesRead=0;
      output.putString("Task","Boxmatic-TransmitData");
      ...
      if(printMessage) {
        InfoUtils.showGuiOrInGameMessage(MessageType.ERROR,"boxmatica.message.error.schematic_transmit_fail");
      }

      output=new NbtCompound();
      output.putLong("SliceKey",sessionKey);
      output.putString("Task","Boxmatic-TransmitCancel");
      ServuxBoxmaticaHandler.getInstance().encodeClientData(ServuxBoxmaticaPacket.ResponseC2SStart(output));
      Boxmatica.LOGGER.error("sliceForServux: Exception reading file; {}",err.getLocalizedMessage());
      return;
    }
    ...
    output.putString("Task","Boxmatic-TransmitEnd");
    ServuxBoxmaticaHandler.getInstance().encodeClientData(ServuxBoxmaticaPacket.ResponseC2SStart(output));
    Boxmatica.debugLog("receiveFileTransmit: Treansmitted file '{}', [tS: {}, tB: {}]",file.toAbsolutePath().toString(),totalSlices,totalBytes);

    if(printMessage) {
      InfoUtils.showGuiOrInGameMessage(MessageType.INFO,"boxmatica.message.schematic_transmit_complete",totalBytes);
    }
  }

  public static @Nullable Pair<BoxmaticaSchematic,NbtCompound> receiveFileTransmit(NbtCompound nbt) {
    ...
    if(task.isEmpty()||key==-1L) {
      Boxmatica.LOGGER.error("receiveFileTransmit: Invalid sessionKey or Task received.");
      return null;
    }
    ...
    switch(task) {
      ...
      case "Boxmatic-TransmitData"-> {
        ...
        if(slice<0||size<0||data.length==0) {
          Boxmatica.LOGGER.error("receiveFileTransmit: Invalid Slice Data received for session key [{}]",key);
          return null;
        }

        manager.receiveSlice(key,slice,data,size);
      }
      case "Boxmatic-TransmitCancel"-> {
        Boxmatica.LOGGER.warn("receiveFileTransmit: Cancel received for session key [{}]",key);
        manager.cancelBuffer(key);
      }
      case "Boxmatic-TransmitEnd"-> {
        ...
        BoxmaticaSchematic schematic=manager.finishBuffer(key,dir);

        if(schematic==null) {
          Boxmatica.LOGGER.warn("receiveFileTransmit: Failed to create Schematic for finishing session key [{}]",key);
          return null;
        }

        // Successful transmission
        Boxmatica.debugLog("receiveFileTransmit: Received file '{}', [tS: {}, tB: {}]",schematic.getFile().toAbsolutePath().toString(),totalSlices,totalSize);
    ...
    if(nbt.contains("Version")) {
      ...
      if(version>=1&&version<=SCHEMATIC_VERSION) {
        if(minecraftDataVersion-this.MINECRAFT_DATA_VERSION>100) {
          InfoUtils.showGuiOrInGameMessage(MessageType.WARNING,"boxmatica.error.schematic_load.newer_minecraft_version",minecraftDataVersion,this.MINECRAFT_DATA_VERSION);
        }
        ...
        InfoUtils.showGuiOrInGameMessage(MessageType.ERROR,"boxmatica.error.schematic_load.unsupported_schematic_version",version);
      }
    }else {
      InfoUtils.showGuiOrInGameMessage(MessageType.ERROR,"boxmatica.error.schematic_load.no_schematic_version_information");
    }
      ...
      if(tag.get(regionName).getType()==Constants.NBT.TAG_COMPOUND) {
        ...
        if(regionPos!=null&&regionSize!=null) {
          ...
          if(nbtBase!=null&&nbtBase.getType()==Constants.NBT.TAG_LONG_ARRAY) {
            ...
            BoxmaticaBlockStateContainer container=BoxmaticaBlockStateContainer.createFrom(palette,blockStateArr,size);
  ...
  protected boolean readPaletteFromBoxmaticaFormatTag(NbtList tagList,IBoxmaticaBlockStatePalette palette) {
  ...
  protected boolean readSpongePaletteFromTag(NbtCompound tag,IBoxmaticaBlockStatePalette palette) {
  ...
  protected boolean readSpongeBlocksFromTag(NbtCompound tag,String schematicName,Vec3i size,int minecraftDataVersion,int spongeVersion) {
    ...
    BoxmaticaBlockStateContainer container=BoxmaticaBlockStateContainer.createContainer(paletteSize,blockData,size);
  ...
  public boolean readFromVanillaStructure(String name,NbtCompound tag) {
    ...
    if(tag.contains("palette")&&
      ...
      if(minecraftDataVersion<BoxmaticaSchematic.MINECRAFT_DATA_VERSION&&effective!=null) {
        Boxmatica.LOGGER.info("VanillaStructure: executing Vanilla DataFixer for Block State Palette DataVersion {} -> {}",minecraftDataVersion,BoxmaticaSchematic.MINECRAFT_DATA_VERSION);
      }else if(effective==null) {
        Boxmatica.LOGGER.warn("readFromVanillaStructure(): Effective Schema has been bypassed.  Not applying Vanilla Data Fixer for Block State Palette DataVersion {}",minecraftDataVersion);
      }
      ...
      // due to how it's currently handled in the Boxmatica container.
      ...
      BoxmaticaBlockStateContainer container=new BoxmaticaBlockStateContainer(size.getX(),size.getY(),size.getZ(),bits,null);
      IBoxmaticaBlockStatePalette palette=container.getPalette();
  ...
  protected List<EntityInfo> readEntitiesFromVanillaStructure(NbtCompound tag,int minecraftDataVersion) {
    ...
    if(minecraftDataVersion<BoxmaticaSchematic.MINECRAFT_DATA_VERSION&&effective!=null) {
      Boxmatica.LOGGER.info("VanillaStructure: executing Vanilla DataFixer for Entities DataVersion {} -> {}",minecraftDataVersion,BoxmaticaSchematic.MINECRAFT_DATA_VERSION);
    }else if(effective==null) {
      Boxmatica.LOGGER.warn("readEntitiesFromVanillaStructure(): Effective Schema has been bypassed.  Not applying Vanilla Data Fixer for Entities DataVersion {}",minecraftDataVersion);
    }
    for(int i=0;i<size;++i) {
  ...
  private void postProcessContainerIfNeeded(NbtList palette,BoxmaticaBlockStateContainer container,@Nullable Map<BlockPos,NbtCompound> tiles) {
  ...
  private NbtList convertBlockStatePalette_to_1_20_5(NbtList oldPalette,int minecraftDataVersion) {
    ...
    if(minecraftDataVersion<BoxmaticaSchematic.MINECRAFT_DATA_VERSION) {
      Schema effective=DataFixerMode.getEffectiveSchema(minecraftDataVersion);

      if(effective==null) {
        Boxmatica.LOGGER.warn("BoxmaticaSchematic: Effective Schema has been bypassed.  Not applying Vanilla Data Fixer for Block State Palette DataVersion {}",minecraftDataVersion);
        return oldPalette;
      }

      NbtList newPalette=new NbtList();
      final int count=oldPalette.size();
      Boxmatica.LOGGER.info("BoxmaticaSchematic: executing Vanilla DataFixer for Block State Palette DataVersion {} -> {}",minecraftDataVersion,BoxmaticaSchematic.MINECRAFT_DATA_VERSION);
  ...
  private Map<BlockPos,NbtCompound> convertTileEntities_to_1_20_5(Map<BlockPos,NbtCompound> oldTE,int minecraftDataVersion) {
    ...
    if(minecraftDataVersion<BoxmaticaSchematic.MINECRAFT_DATA_VERSION) {
      ...
      if(effective==null) {
        Boxmatica.LOGGER.warn("BoxmaticaSchematic: Effective Schema has been bypassed.  Not applying Vanilla Data Fixer for Tile Entities DataVersion {}",minecraftDataVersion);
        return oldTE;
      }

      Map<BlockPos,NbtCompound> newTE=new HashMap<>();

      Boxmatica.LOGGER.info("BoxmaticaSchematic: executing Vanilla DataFixer for Tile Entities DataVersion {} -> {}",minecraftDataVersion,BoxmaticaSchematic.MINECRAFT_DATA_VERSION);
  ...
  private NbtList convertEntities_to_1_20_5(NbtList oldEntitiesList,int minecraftDataVersion) {
    ...
    if(minecraftDataVersion<BoxmaticaSchematic.MINECRAFT_DATA_VERSION) {
      ...
      if(effective==null) {
        Boxmatica.LOGGER.warn("BoxmaticaSchematic: Effective Schema has been bypassed.  Not applying Vanilla Data Fixer for Entities DataVersion {}",minecraftDataVersion);
      ...
      Boxmatica.LOGGER.info("BoxmaticaSchematic: executing Vanilla DataFixer for Entities DataVersion {} -> {}",minecraftDataVersion,BoxmaticaSchematic.MINECRAFT_DATA_VERSION);
  ...
  private List<EntityInfo> convertSpongeEntities_to_1_20_5(List<EntityInfo> oldEntitiesList,int minecraftDataVersion) {
    ...
    if(minecraftDataVersion<BoxmaticaSchematic.MINECRAFT_DATA_VERSION) {
      ...
      if(effective==null) {
        Boxmatica.LOGGER.warn("SpongeSchematic: Effective Schema has been bypassed.  Not applying Vanilla Data Fixer for Entities DataVersion {}",minecraftDataVersion);
        return oldEntitiesList;
      }

      List<EntityInfo> newEntitiesList=new ArrayList<>();

      Boxmatica.LOGGER.info("SpongeSchematic: executing Vanilla DataFixer for Entities DataVersion {} -> {}",minecraftDataVersion,BoxmaticaSchematic.MINECRAFT_DATA_VERSION);
  ...
  private Map<BlockPos,NbtCompound> downgradeTileEntities_to_1_20_4(Map<BlockPos,NbtCompound> oldTE,int minecraftDataVersion) {
    ...
    Boxmatica.LOGGER.info("BoxmaticaSchematic: Downgrade Tile Entities from DataVersion {} -> {}",minecraftDataVersion,BoxmaticaSchematic.MINECRAFT_DATA_VERSION_1_20_4);
  ...
  private NbtList downgradeEntities_to_1_20_4(NbtList oldEntitiesList,int minecraftDataVersion) {
    ...
    Boxmatica.LOGGER.info("BoxmaticaSchematic: Downgrade Entities from DataVersion {} -> {}",minecraftDataVersion,BoxmaticaSchematic.MINECRAFT_DATA_VERSION_1_20_4);
  ...
  private List<EntityInfo> readEntitiesFromNBT(NbtList tagList) {
    ...
    for(int i=0;i<size;++i) {
      ...
      //            Boxmatica.LOGGER.error("readEntitiesFromNBT: posVec [{}], NBT [{}]", posVec.toString(), entityData.toString());
  ...
  public boolean writeToFile(Path dir,String fileNameIn,boolean override,boolean downgrade) {
    ...
    try {
      ...
      if(!Files.isDirectory(dir)) {
        InfoUtils.showGuiOrInGameMessage(MessageType.ERROR,"boxmatica.error.schematic_write_to_file_failed.directory_creation_failed",dir.toAbsolutePath());
        return false;
      }

      if(override==false&&Files.exists(fileSchematic)) {
        InfoUtils.showGuiOrInGameMessage(MessageType.ERROR,"boxmatica.error.schematic_write_to_file_failed.exists",fileSchematic.toAbsolutePath());
        return false;
      }
      ...
      InfoUtils.showGuiOrInGameMessage(MessageType.ERROR,"boxmatica.error.schematic_write_to_file_failed.exception",fileSchematic.toAbsolutePath());
      Boxmatica.LOGGER.error(StringUtils.translate("boxmatica.error.schematic_write_to_file_failed.exception",fileSchematic.toAbsolutePath()),e);
      Boxmatica.LOGGER.error(e.getMessage());
    }
  ...
  private boolean readFromFile(FileType schematicType) {
    try {
      ...
      if(nbt!=null) {
        ...
        InfoUtils.showGuiOrInGameMessage(MessageType.ERROR,"boxmatica.error.schematic_read_from_file_failed.cant_read",this.schematicFile.toAbsolutePath());
      }
    }catch(Exception e) {
      InfoUtils.showGuiOrInGameMessage(MessageType.ERROR,"boxmatica.error.schematic_read_from_file_failed.exception",this.schematicFile.toAbsolutePath());
      Boxmatica.LOGGER.error(e);
    }
  ...
  public static NbtCompound readNbtFromFile(Path file) {
    if(file==null) {
      InfoUtils.showGuiOrInGameMessage(MessageType.ERROR,"boxmatica.error.schematic_read_from_file_failed.no_file");
      return null;
    }

    if(Files.exists(file)==false||Files.isReadable(file)==false) {
      InfoUtils.showGuiOrInGameMessage(MessageType.ERROR,"boxmatica.error.schematic_read_from_file_failed.cant_read",file.toAbsolutePath());
      return null;
    }
  ...
  public static void updateMetadataWithFileTime(Path file,SchematicMetadata metadata) {
    try {
      ...
      Boxmatica.LOGGER.error("getFileCreatedTime(): Exception reading file '{}'; {}",file.getFileName().toString(),err.getLocalizedMessage());
    }
  }
    ...
    if(nbt!=null) {
      switch(type) {
        ...
        case SPONGE_SCHEMATIC-> {
          BoxmaticaSchematic schem=new BoxmaticaSchematic(file,type);
  ...
  @Nullable
  public static BoxmaticaSchematic createFromFile(Path dir,String fileName,FileType schematicType) {
    Path file=fileFromDirAndName(dir,fileName,schematicType);
    BoxmaticaSchematic schematic=new BoxmaticaSchematic(file,schematicType);
  ...
  public static class EntityInfo{
    ...
    public EntityInfo(Vec3d posVec,NbtCompound nbt) {
      ...
      if(nbt.contains("SleepingX")) {
        nbt.putInt("SleepingX",MathHelper.floor(posVec.x));
      }
      if(nbt.contains("SleepingY")) {
        nbt.putInt("SleepingY",MathHelper.floor(posVec.y));
      }
      if(nbt.contains("SleepingZ")) {
        nbt.putInt("SleepingZ",MathHelper.floor(posVec.z));
      }
  ...
  public String toString() {
    ...
    return "BoxmaticaSchematic["+nbt.toString()+"]";
  }
}
