...
public class SchematicConversionMaps{
  ...
  private static void computeMapsLegacy() {
    for(ConversionData data:CACHED_DATA) {
      try {
        ...
        Boxmatica.LOGGER.warn("computeMapsLegacy(): Exception while adding blockstate conversion map entry for ID '{}' (fixed state: '{}')",data.idMeta,data.newStateString,e);
      }
    }
  }
  ...
  private static void computeMapsDynamic() {
    for(ConversionDynamic entry:CACHED_DYNAMIC) {
      try {
        ...
        Boxmatica.LOGGER.warn("computeMapsDynamic(): Exception while adding blockstate conversion map entry for ID '{}' (fixed state: '{}')",entry.idMeta,entry.newState.toString(),e);
      }
    }
  }
  ...
  private static void addIdMetaToBlockState(int idMeta,NbtCompound newStateTag,String... oldStateStrings) {
    try {
      ...
      Boxmatica.LOGGER.warn("addIdMetaToBlockState(): Exception while adding blockstate conversion map entry for ID '{}'",idMeta,e);
    }
  }
  ...
  private static void addIdMetaToBlockStateDynamic(int idMeta,NbtCompound newStateTag,List<Dynamic<?>> oldStates) {
    try {
      ...
      Boxmatica.LOGGER.warn("addIdMetaToBlockStateDynamic(): Exception while adding blockstate conversion map entry for ID '{}'",idMeta,e);
    }
  }
  ...
  private static void addOldStateToNewState(NbtCompound newStateTagIn,String... oldStateStrings) {
    try {
      ...
      Boxmatica.LOGGER.warn("addOldStateToNewState(): Exception while adding new blockstate to old blockstate conversion map entry for '{}'",newStateTagIn,e);
    }
  }
    ...
    try {
      ...
      if(oldStates.size()==1) {
        ...
        try {
          ...
          Boxmatica.LOGGER.warn("addOldStateToNewStateDynamic(): Exception while adding new blockstate to old blockstate conversion map entry for '{}'",newStateTagIn,err);
        }
      ...
      Boxmatica.LOGGER.warn("addOldStateToNewStateDynamic(): Exception while adding new blockstate to old blockstate conversion map entry for '{}'",newStateTagIn,e);
    }
  }
  ...
  public static String updateBlockName(String oldName,int oldVersion) {
    ...
    try {
      return MinecraftClient.getInstance().getDataFixer()
        .update(TypeReferences.BLOCK_NAME,new Dynamic<>(NbtOps.INSTANCE,tagStr),oldVersion,BoxmaticaSchematic.MINECRAFT_DATA_VERSION)
        .getValue().asString()
        .orElse(oldName);
    }catch(Exception e) {
      Boxmatica.LOGGER.warn("updateBlockName: failed to update Block Name [{}], preserving original state (data may become lost)",oldName);
      return oldName;
    }
  }
  ...
  public static NbtCompound updateBlockStates(NbtCompound oldBlockState,int oldVersion) {
    try {
      return (NbtCompound)MinecraftClient.getInstance().getDataFixer().update(TypeReferences.BLOCK_STATE,new Dynamic<>(NbtOps.INSTANCE,oldBlockState),oldVersion,BoxmaticaSchematic.MINECRAFT_DATA_VERSION).getValue();
    }catch(Exception e) {
      Boxmatica.LOGGER.warn("updateBlockStates: failed to update Block State [{}], preserving original state (data may become lost)",
  ...
  public static NbtCompound updateBlockEntity(NbtCompound oldBlockEntity,int oldVersion) {
    try {
      return (NbtCompound)MinecraftClient.getInstance().getDataFixer().update(TypeReferences.BLOCK_ENTITY,new Dynamic<>(NbtOps.INSTANCE,oldBlockEntity),oldVersion,BoxmaticaSchematic.MINECRAFT_DATA_VERSION).getValue();
    }catch(Exception e) {
      BlockPos pos=NbtUtils.readBlockPos(oldBlockEntity);
      Boxmatica.LOGGER.warn("updateBlockEntity: failed to update Block Entity [{}] at [{}], preserving original state (data may become lost)",
  ...
  public static NbtCompound updateEntity(NbtCompound oldEntity,int oldVersion) {
    try {
      return (NbtCompound)MinecraftClient.getInstance().getDataFixer().update(TypeReferences.ENTITY,new Dynamic<>(NbtOps.INSTANCE,oldEntity),oldVersion,BoxmaticaSchematic.MINECRAFT_DATA_VERSION).getValue();
    }catch(Exception e) {
      Boxmatica.LOGGER.warn("updateEntity: failed to update Entity [{}], preserving original state (data may become lost)",
