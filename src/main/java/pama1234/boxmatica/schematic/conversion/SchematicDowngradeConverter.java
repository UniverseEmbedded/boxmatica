  ...
  private static NbtElement processSkullProfile(NbtElement oldProfile,NbtCompound dispNbt,int minecraftDataVersion,@Nonnull DynamicRegistryManager registryManager) {
    ...
    if(name.isEmpty()&&!customName1.isEmpty()) {
      try {
        ...
        Boxmatica.LOGGER.warn("processSkullProfile(): Exception deserializing CustomName1 for Head Name.");
        name=customName1;
      }
      ...
      try {
        ...
        Boxmatica.LOGGER.warn("processSkullProfile(): Exception deserializing CustomName2 for Head Name.");
        name=customName2;
      }
    }
