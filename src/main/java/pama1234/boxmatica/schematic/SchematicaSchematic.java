...
import pama1234.boxmatica.schematic.BoxmaticaSchematic.EntityInfo;
import pama1234.boxmatica.schematic.container.BoxmaticaBlockStateContainer;
import pama1234.boxmatica.schematic.conversion.SchematicConversionFixers.IStateFixer;
...
import pama1234.boxmatica.util.DataFixerMode;
...
public class SchematicaSchematic{
  ...
  private BoxmaticaBlockStateContainer blocks;
  ...
  private void readBlocksFromWorld(World world,BlockPos posStart,BlockPos size) {
    ...
    this.blocks=new BoxmaticaBlockStateContainer(size.getX(),size.getY(),size.getZ());
    this.tiles.clear();
    this.size=size;
        ...
        for(int x=startX;x<endX;++x) {
          ...
          if(te!=null) {
            try {
              ...
              Boxmatica.LOGGER.warn("SchematicaSchematic: Exception while trying to store TileEntity data for block '{}' at {}",
  ...
  public boolean readFromNBT(NbtCompound nbt) {
    if(this.readBlocksFromNBT(nbt)) {
      ...
      try {
        ...
        Boxmatica.LOGGER.error("SchematicaSchematic: Exception while post-processing blocks for '{}'",this.fileName,e);
      }

      return true;
    }else {
      Boxmatica.LOGGER.error("SchematicaSchematic: Missing block data in the schematic '{}'",this.fileName);
      return false;
    }
  }
  ...
  private boolean readBlocksFromNBT(NbtCompound nbt) {
    ...
    this.size=new Vec3i(sizeX,sizeY,sizeZ);
    this.blocks=new BoxmaticaBlockStateContainer(sizeX,sizeY,sizeZ);
    this.metadata.setEnclosingSize(this.size);
    this.metadata.setTotalBlocks(numBlocks);
    this.metadata.setTotalVolume(sizeX*sizeY*sizeZ);
    this.metadata.setRegionCount(1);
  ...
  private void readEntitiesFromNBT(NbtCompound nbt) {
    ...
    if(effective!=null) {
      Boxmatica.LOGGER.info("SchematicaSchematic: executing Vanilla DataFixer for Entities DataVersion {} -> {}",minecraftDataVersion,BoxmaticaSchematic.MINECRAFT_DATA_VERSION);
    }else {
      Boxmatica.LOGGER.warn("SchematicaSchematic: Effective Schema has been bypassed.  Not applying Vanilla Data Fixer for Entities DataVersion {}",minecraftDataVersion);
    }
  ...
  private void readTileEntitiesFromNBT(NbtCompound nbt) {
    ...
    if(effective!=null) {
      Boxmatica.LOGGER.info("SchematicaSchematic: executing Vanilla DataFixer for Tile Entities DataVersion {} -> {}",minecraftDataVersion,BoxmaticaSchematic.MINECRAFT_DATA_VERSION);
    }else {
      Boxmatica.LOGGER.warn("SchematicaSchematic: Effective Schema has been bypassed.  Not applying Vanilla Data Fixer for Tile Entities DataVersion {}",minecraftDataVersion);
    }
  ...
  public boolean readFromFile(Path file) {
    if(Files.exists(file)&&Files.isRegularFile(file)&&Files.isReadable(file)) {
      ...
      try {
        ...
        Boxmatica.LOGGER.error("SchematicaSchematic: Failed to read Schematic data from file '{}'",file.toAbsolutePath());
      }
    }
   ...
   * IBoxmaticaBlockStatePalette boxmaticaPalette = this.blocks.getPalette();
   * final int numBlocks = boxmaticaPalette.getPaletteSize();
   * 
   * for (int i = 0; i < numBlocks; ++i)
   * {
   * IBlockState state = boxmaticaPalette.getBlockState(i);
   ...
   * format("Block id %d for block '%s' is out of range, max allowed = %d!",
   ...
   * }
   * 
   * public boolean writeToFile(File dir, String fileNameIn, boolean override, IStringConsumer
   * feedback)
   * {
   ...
   * {
   * feedback.setString(StringUtils.translate(
   * "boxmatica.error.schematic_write_to_file_failed.directory_creation_failed",
   * dir.getAbsolutePath()));
   * return false;
   * }
   * 
   ...
   * {
   * feedback.setString(StringUtils.translate(
   * "boxmatica.error.schematic_write_to_file_failed.exists", fileSchematic.getAbsolutePath()));
   * return false;
   * }
   * 
   ...
   * {
   * feedback.setString(StringUtils.translate(
   * "boxmatica.error.schematic_write_to_file_failed.exception",
   * fileSchematic.getAbsolutePath()));
   * }
   * 
   * return false;
   * }
