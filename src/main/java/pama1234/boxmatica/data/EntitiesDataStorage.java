...
public class EntitiesDataStorage implements IClientTickHandler,IDataSyncer{
  ...
  private final static ServuxBoxmaticaHandler<ServuxBoxmaticaPacket.Payload> HANDLER=ServuxBoxmaticaHandler.getInstance();
    ...
    if(now-this.serverTickTime>50) {
      ...
      if(Configs.Generic.ENTITY_DATA_SYNC.getBooleanValue()==false) {
        ...
        this.getWorld()!=null) {
          ...
          HANDLER.registerPlayReceiver(ServuxBoxmaticaPacket.Payload.ID,HANDLER::receivePlayPayload);
  ...
  public Identifier getNetworkChannel() {
    return ServuxBoxmaticaHandler.CHANNEL_ID;
  }
  ...
  public IPluginClientPlayHandler<ServuxBoxmaticaPacket.Payload> getNetworkHandler() {
    return HANDLER;
  }
  ...
  public void reset(boolean isLogout) {
    if(isLogout) {
      Boxmatica.debugLog("EntitiesDataStorage#reset() - log-out");
      ...
      Boxmatica.debugLog("EntitiesDataStorage#reset() - dimension change or log-in");
      long now=System.currentTimeMillis();
  ...
  private void tickCache(long nowTime) {
    ...
    // Use LongTimeouts when saving a Boxmatic Selection,
  ...
  public void setServuxVersion(String ver) {
    if(ver!=null&&ver.isEmpty()==false) {
      ...
      Boxmatica.debugLog("BoxmaticDataChannel: joining Servux version {}",ver);
    }else {
  ...
  public void onGameInit() {
    ...
    HANDLER.registerPlayPayload(ServuxBoxmaticaPacket.Payload.ID,ServuxBoxmaticaPacket.Payload.CODEC,IPluginClientPlayHandler.BOTH_CLIENT);
  }
  ...
  public void requestMetadata() {
    if(DataManager.getInstance().hasIntegratedServer()==false&&
      ...
      HANDLER.encodeClientData(ServuxBoxmaticaPacket.MetadataRequest(nbt));
    }
  }

  public boolean receiveServuxMetadata(NbtCompound data) {
    if(DataManager.getInstance().hasIntegratedServer()==false) {
      Boxmatica.debugLog("BoxmaticDataChannel: received METADATA from Servux");

      if(Configs.Generic.ENTITY_DATA_SYNC.getBooleanValue()) {
        if(data.getInt("version",-1)!=ServuxBoxmaticaPacket.PROTOCOL_VERSION) {
          Boxmatica.LOGGER.warn("BoxmaticDataChannel: Mis-matched protocol version!");
        }
    ...
    if(this.blockEntityCache.containsKey(pos)) {
      ...
      if(!DataManager.getInstance().hasIntegratedServer()&&
        ...
        if(System.currentTimeMillis()-this.blockEntityCache.get(pos).getLeft()>(this.getCacheTimeout()/4)) {
          //                    Boxmatica.debugLog("requestBlockEntity: be at pos [{}] requeue at [{}] ms", pos.toShortString(), this.getCacheTimeout() / 4);
      ...
      if(DataManager.getInstance().hasIntegratedServer()==false&&
        (Configs.Generic.ENTITY_DATA_SYNC.getBooleanValue()||
          Configs.Generic.ENTITY_DATA_SYNC_BACKUP.getBooleanValue())) {
        //                Boxmatica.debugLog("requestBlockEntity: be at pos [{}] queue at [{}] ms", pos.toShortString(), this.getCacheTimeout() / 4);
        this.pendingBlockEntitiesQueue.add(pos);
      }
    ...
    if(this.entityCache.containsKey(entityId)) {
      ...
      if(!DataManager.getInstance().hasIntegratedServer()&&
        ...
        if(System.currentTimeMillis()-this.entityCache.get(entityId).getLeft()>(this.getCacheTimeout()/4)) {
          //Boxmatica.debugLog("requestEntity: entity Id [{}] requeue at [{}] ms", entityId, this.getCacheTimeout() / 4);
  ...
  private void requestServuxBlockEntityData(BlockPos pos) {
    if(Configs.Generic.ENTITY_DATA_SYNC.getBooleanValue()) {
      HANDLER.encodeClientData(ServuxBoxmaticaPacket.BlockEntityRequest(pos));
    }
  }

  private void requestServuxEntityData(int entityId) {
    if(Configs.Generic.ENTITY_DATA_SYNC.getBooleanValue()) {
      HANDLER.encodeClientData(ServuxBoxmaticaPacket.EntityRequest(entityId));
    }
  }
  ...
  public void requestServuxBulkEntityData(ChunkPos chunkPos,int minY,int maxY) {
    ...
    Boxmatica.debugLog("EntitiesDataStorage#requestServuxBulkEntityData(): for chunkPos [{}] to Servux (minY [{}], maxY [{}])",chunkPos.toString(),minY,maxY);
    HANDLER.encodeClientData(ServuxBoxmaticaPacket.BulkNbtRequest(chunkPos,req));
  }
  ...
  public void requestBackupBulkEntityData(ChunkPos chunkPos,int minY,int maxY) {
    ...
    Boxmatica.debugLog("EntitiesDataStorage#requestBackupBulkEntityData(): for chunkPos {} (minY [{}], maxY [{}]) // Request --> TE: [{}], E: [{}]",chunkPos.toString(),minY,maxY,teSet.size(),entList.size());
  ...
  private boolean markBackupBlockEntityComplete(ChunkPos chunkPos,BlockPos pos) {
    ...
    //Boxmatica.debugLog("EntitiesDataStorage#markBackupBlockEntityComplete() - Marking ChunkPos {} - Block Entity at [{}] as complete.", chunkPos.toString(), pos.toShortString());
    ...
    if(this.pendingChunks.contains(chunkPos)) {
      if(this.pendingBackupChunk_BlockEntities.containsKey(chunkPos)) {
        ...
        if(teSet.contains(pos)) {
          ...
          if(teSet.isEmpty()) {
            Boxmatica.debugLog("EntitiesDataStorage#markBackupBlockEntityComplete(): ChunkPos {} - Block Entity List Complete!",chunkPos.toString());
  ...
  private boolean markBackupEntityComplete(ChunkPos chunkPos,int entityId) {
    ...
    //Boxmatica.debugLog("EntitiesDataStorage#markBackupEntityComplete() - Marking ChunkPos {} - EntityId [{}] as complete.", chunkPos.toString(), entityId);
    ...
    if(this.pendingChunks.contains(chunkPos)) {
      if(this.pendingBackupChunk_Entities.containsKey(chunkPos)) {
        ...
        if(entSet.contains(entityId)) {
          ...
          if(entSet.isEmpty()) {
            Boxmatica.debugLog("EntitiesDataStorage#markBackupEntityComplete(): ChunkPos {} - EntitiyList Complete!",chunkPos.toString());
  ...
  public void handleBulkEntityData(int transactionId,@Nullable NbtCompound nbt) {
    ...
    if(task.equals("BulkEntityReply")) {
      ...
      Boxmatica.debugLog("EntitiesDataStorage#handleBulkEntityData(): [ChunkPos {}] received TE: [{}], and E: [{}] entiries from Servux",chunkPos.toString(),tileList.size(),entityList.size());
    }
  }
  ...
  private void checkForPendingChunkTimeout(ChunkPos pos) {
    if((this.hasServuxServer()&&this.hasPendingChunk(pos))||
      ...
      if(duration>(this.getChunkTimeoutMs())) {
        Boxmatica.debugLog("EntitiesDataStorage#checkForPendingChunkTimeout(): [ChunkPos {}] has timed out waiting for data, marking complete without Receiving Entity Data.",pos.toString());
