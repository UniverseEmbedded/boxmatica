...
public class LitematicaBlockStateContainer implements ILitematicaBlockStatePaletteResizer{
  private static final Codec<ILitematicaBlockStatePalette> PALETTE_CODEC=Codec.either(LitematicaBlockStatePaletteHashMap.CODEC,LitematicaBlockStatePaletteLinear.CODEC)
    .xmap(
      ...
      type->type instanceof LitematicaBlockStatePaletteHashMap hashMap?Either.left(hashMap):type instanceof LitematicaBlockStatePaletteLinear linear?Either.right(linear):null);
  public static final Codec<LitematicaBlockStateContainer> CODEC=RecordCodecBuilder.create(
    inst->inst.group(
      ...
      PrimitiveCodec.LONG_STREAM.optionalFieldOf("BlockCounts",LongStream.empty()).forGetter(get->Arrays.stream(get.blockCounts))).apply(inst,LitematicaBlockStateContainer::new));
